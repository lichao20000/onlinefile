...
public class Session {
	...
	/**
	 * Longest allowable inactivity period (in seconds). 10秒
	 */
	public static final int MAX_INACTIVITY = 10;
	...
	private static String createSessionID(int len) {
		...
		LOGGER.error("The Method Is createSessionID() :" + str);
	...
	public Session(String to, String route) throws UnknownHostException,
		...
		try {
			...
			LOGGER.error("The Method Is Session(String to, String route) :" + e.getMessage());
		}
		...
		if (route != null && !route.equals("")) {
			...
			if ((i = route.lastIndexOf(":")) != -1) {
				try {
					...
					LOGGER.error("The Method Is Session(String to, String route) :" + nfe.getMessage());
				}

				route = route.substring(0, i);
			}

			DebugLog.dbg("Trying to open a socket to '" + route
					+ "', using port " + port + ".", 3);
		...
		if (this.sock == null || !this.sock.isConnected()) {
			this.sock = new Socket();
			DebugLog.dbg("Trying to use 'to' attribute to open a socket...",
					3);
			host = DNSUtil.resolveXMPPServerDomain(to, DEFAULT_XMPPPORT);
			try {
				DebugLog.dbg("Trying to open a socket to '" + host.getHost()
				...
				LOGGER.error("The Method Is Session(String to, String route) :" + uhe.getMessage());
				throw uhe;
			} catch (IOException ioe) {
				DebugLog.dbg(
							   "Failed to open a socket using the 'to' attribute: " + ioe.toString(), 3);
				LOGGER.error("The Method Is Session(String to, String route) :" + ioe.getMessage());
				throw ioe;
			}
		}
		...
		try {
			if (this.sock.isConnected())
				DebugLog.dbg("Succesfully connected to " + to, 2);
			...
			DebugLog.dbg("creating session with id " + this.sid, 2);
	...
	public NodeList checkInQ(long rid) throws IOException {
		...
		DebugLog.dbg("inQueue: " + inQueue, 2);
		...
		if (init_retry < 1000 && (this.authid == null || this.isReinit())
			...
			if (stream10Test.matcher(inQueue).matches()) {
				...
				if (m.matches()) {
					...
					DebugLog.dbg("failed to get stream features", 2);
					try {
						Thread.sleep(5);
					} catch (InterruptedException ie) {
						LOGGER.error("The Method Is checkInQ(long rid) :" + ie.getMessage());
					}
				...
				if (m.matches()) {
					...
					DebugLog.dbg("failed to get authid", 2);
					try {
						Thread.sleep(5);
					} catch (InterruptedException ie) {
						LOGGER.error("The Method Is checkInQ(long rid) :" + ie.getMessage());
			...
			try {
				...
				if (streamFeatures)
					...
					try {
						...
						LOGGER.error("The Method Is checkInQ(long rid) :" + sex.getMessage());
						try {
							// stream closed?
							doc = db.parse(new InputSource(new StringReader(
							...
							LOGGER.error("The Method Is checkInQ(long rid) :" + sex2.getMessage());
						}
					}
				if (doc != null)
						...
						if (nl.item(0).getChildNodes().item(i).getNodeName()
							...
							if (!this.isReinit()) {
								...
								try {
									...
									DebugLog.dbg("initiating handshake");
									tls.startHandshake();
									try {
										...
										LOGGER.error("The Method Is checkInQ(long rid) :" + ire.getMessage());
									}

									DebugLog.dbg("TLS Handshake complete", 2);
									...
									DebugLog.dbg("STARTTLS failed: "
											+ ssle.toString(), 1);
									this.setReinit(false);
									LOGGER.error("The Method Is checkInQ(long rid) :" + ssle.getMessage());
									if (this.isSecure()) {
				...
				LOGGER.error("The Method Is checkInQ(long rid) :" + sex3.getMessage());
				this.setReinit(false);
				DebugLog.dbg("failed to parse inQueue: " + inQueue + "\n"
						+ sex3.toString(), 1);
				return null;
			}
		}
	...
	private class HandShakeFinished implements
		...
		public void handshakeCompleted(
			...
			DebugLog.dbg("startTLS: Handshake is complete", 2);
	...
	public synchronized boolean checkValidRID(long rid) {
		try {
			if (rid <= ((Long) this.responses.lastKey()).longValue()
				...
				DebugLog.dbg("invalid request id: " + rid + " (last: "
						+ ((Long) this.responses.lastKey()).longValue() + ")",
						1);
				return false;
			}
		} catch (NoSuchElementException e) {
			LOGGER.error("The Method Is checkValidRID(long rid) :" + e.getMessage());
			return false;
		}
	}
			...
			try {
				if (this.br.ready()) {
					...
					if ((this.hold == 0 && r != null && System
						...
						DebugLog.dbg("readFromSocket done for " + rid, 3);
						break; // time exeeded
					}
					...
					try {
						...
						LOGGER.error("The Method Is readFromSocket(long rid) :" + ie.getMessage());
					}
				}
			} catch (IOException e) {
				System.err.println("Can't read from socket");
				LOGGER.error("The Method Is readFromSocket(long rid) :" + e.getMessage());
				this.terminate();
			}
		}
	...
	public Session sendNodes(NodeList nl) {
		...
		try {
			...
			DebugLog.dbg("XML.toString(Document): " + e, 1);
		}

		try {
			if (this.isReinit()) {
				DebugLog.dbg("Reinitializing Stream!", 2);
				this.osw.write("<stream:stream to='" + this.to + "'"
						+ " xmlns='jabber:client' "
						+ " xmlns:stream='http://etherx.jabber.org/streams'"
						+ " version='1.0'" + ">");
			...
			DebugLog.dbg(this.sid + " failed to write to stream", 1);
			LOGGER.error("The Method Is sendNodes(NodeList nl) :" + ioe.getMessage());
		}
...
//		DebugLog.dbg("terminating session " + this.getSID(), 3);
		...
		synchronized (this.sock) {
			if (!this.sock.isClosed()) {
				try {
					...
					LOGGER.error("The Method Is terminate :" + ie.getMessage());
				}
			}
			this.sock.notifyAll();
		}
		...
		if(this.username!=null){
			String realusername = this.username.replace("\\40", "@") ;
			BroadcastUtils.broadcast(this.username, "company"+this.companyId+"@broadcast."+BroadcastUtils.openfireServerName, "broadcast-offline:"+realusername, Message.Type.GROUP_CHAT, this.fullname);
		}
	}
