...
public class Response {
	...
	public synchronized void send(HttpServletResponse response) {
		...
		try {
			...
			DebugLog.dbg("sending response ["+this.getRID()+"]: "+strResult.getWriter().toString(),2);
			response.getWriter().println(strResult.getWriter().toString());
			DebugLog.dbg("sent response for "+this.getRID(),3);
		} catch (Exception e) {
	...
	public synchronized void setStatus(String status) {
		DebugLog.dbg("response status "+status+" for "+this.getRID(),3);
		this.status = status;
	}
