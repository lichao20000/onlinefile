$(function(){
	...
	/**    此处判断一下，当version数组长度小于1时候不走下面的判断，不然低版本谷歌浏览器会报错（包含XP）   **/
	...
	$("#activateAccount").on('click',function(){
		...
		var url = $.appClient.generateUrl({ESUserInfo:'activateAccountByLink'},'x');
		var fullname = $("#fullname").val() ;
		...
		$.ajax({
			url:url,
			type:'POST',
			data:{id:$("#userid").val(), username:username,password:password,companyid:companyid,fullname:fullname,companyName:companyName,className:className,classId:classId,sendUserName:sendUserName},
...
//					var arg = 'type=add&username='+username+'&password='+password+'&name='+fullname+'&secret=flyingsoft';
	...
	$('#username').on("blur", function(){
		if($("#alerterror").attr('isAccounted') == "true"){
	 	...
	 	email = $.trim(email);
	 	if(email==''){
	 		$("#usernamealerterror").html("姓名不能为空.");
	 		$("#usernamealerterror").css('display','block');
