...
(function ($) {
	$.addFlex = function (t, p) {
		...
		/** xiaoxiong 20140524 添加更多菜单处理方法 TODO 暂时只支持一个更多菜单 **/
		var moreMenu = {
				hideMenu: function(){
					$("#morelist").hide();
				},
				...
				renderToBody: function(){
					$("body").die().live("click", moreMenu.onBodyDown);
				},
				dealTooltip:function(){//统一处理 更多的title 修改为从更多菜单中动态获取
					var tooltip='包含';
					$("#morelist div[class='fbutton']").each(function(index){
						tooltip+=(index==0?'':',')+$(this).text();
		...
		var g = {
			...
			fixHeight: function (newH) {
				...
				/** xiaoxiong 20140806 修改为>=，避免在修改列宽时，会使得一部分列字段看不到 **/
...
//							}
			...
			//xiaoxiong 20140809 更新一行的数据
			reloadOneTrDataFun: function (trObj, data){
				var that = this;
				$('.pReload', that.pDiv).removeClass('loading');
				this.loading = false;
				if (!data) {
...
//					p.page = 1;
				...
				if (p.dataType == 'json') {
					/** xiaoxiong 20140528 优化数据组装代码逻辑，提升数据展现性能 **/
					var tr ;
					$.each(data.rows, function (i, row) {
						row._rowNum = i;
						if(i==0){
//							tr = g.generateTrByJson(row);
//							oFrag.appendChild(tr);
							/** xiaoxiong 20140728 为了支持IE8 **/
							tr = g.generateInitTrByJson(row) ;
							var newtr = g.generateTrReloadData(row,tr,true);
							oFrag.appendChild(newtr);
							newtr = null;
						} else {
							var newtr = g.generateTrReloadData(row,tr,false);
							oFrag.appendChild(newtr);
							newtr = null;
						}
						//$(tbody).append(tr);
				...
				$(t).empty();
				t.appendChild(tbody);
//				this.addCellProp();/** xiaxoiong 20140528 优化数据组装代码逻辑，提升数据展现性能；暂时将其注释掉，看有没有问题 **/
				this.addRowProp();
				this.rePosDrag();
				tbody = null;
				data = null;
			...
			/** xiaoxiong 20140728 为了支持IE8 **/
			generateInitTrByJson: function (data) {
				...
				thObj.each( //add cell
					function (i) {
						...
						/** xiaoxiong 20140821 添加列模型是否存在判断，避免出现在IE8下显示异常 **/
						if (typeof p.colModel[idx] != "undefined") {
							/**shimiao 20140804 改变颜色*/
//							if('changeColor' == p.colModel[idx].name){
//								if(data.cell[p.colModel[idx].name] !=null && data.cell[p.colModel[idx].name] .indexOf("#")>-1){
//									tr.removeClass='erow';
//									tr.style.background=  data.cell[p.colModel[idx].name];
//								}
//							}
							td.setAttribute('colname', p.colModel[idx].name); //td上添加列名便于操作
							td.setAttribute('abbr',this.getAttribute('abbr'));
							td.setAttribute('maxNumber',this.maxNumber);//设置字段最大值
							td.onclick=function(){
								if(p.onClick){
									p.onClick(this, g, p);
								}
			...
			/** xiaoxiong 20140528 优化数据组装代码逻辑，提升数据展现性能 **/
			generateTrReloadData: function (data, tr, first) {
				var newtr = tr.cloneNode(true); 
				if(!first){
					if (data.name) newtr.name = data.name;
					if (data.color) {
						newtr.style.background=data.color;
					} else {
						if (data._rowNum % 2 && p.striped) newtr.className = 'erow';
					}
					newtr.id = 'row' + data[p.idProperty];
				}
				newtr.data = data;
				ths = $('td', newtr);
				ths.each(
...
//						if(!first){
							...
							if (typeof data.cell == 'undefined') {//.children()[0]
								$(this).children()[0].innerHTML = data[colname];
							} else {
								if (typeof data.cell[i] != "undefined") {
									$(this).children()[0].innerHTML = (data.cell[i] != null) ? data.cell[i] : '&nbsp;';//null-check for Opera-browser
								} else {
									$(this).children()[0].innerHTML = typeof data.cell[colname] != 'undefined' && data.cell[colname] != null ? data.cell[colname] : '&nbsp;';
								}
							}
//							alert($(this).children()[0].innerHTML);
							/**shimiao 20140804 改变颜色*/
							if('changeColor' == colname){
								if($(this).children()[0].innerHTML!=null && $(this).children()[0].innerHTML.indexOf("#")>-1){
									$(this).closest("tr").removeClass('erow');
									$(this).closest("tr").css('background-color',$(this).children()[0].innerHTML);
								}
							}
//						}
						//编辑组件
						if ($(this).attr("editable")) {
							var tdDiv = $(this).children()[0] ;
							$(tdDiv).prop({value:$(tdDiv).text()});
							$(this).click(function(){
								if($(tdDiv).prop("editing")) return;
								var pth = $('th:eq(' + i + ')', g.hDiv).get(0);
								if(pth.dropdown){
						...
						//xiaoxiong 调用process 20140809 
						var pth = $('th:eq(' + i + ')', g.hDiv).get(0);
						if (pth != null) {
							if (pth.process) pth.process($(this).children()[0], data[p.idProperty]);
						}
					}
				);
				return newtr;
			},
			/** xiaoxiong 20140809 更新一行数据的值 **/
			reloadTrData: function (tr, data) {
				ths = $('td', tr);
				ths.each(
					function (i) {
						var colname = $(this).attr("colname");
						/**shimiao 20140804 改变颜色*/
						if('changeColor' == colname){
							if(data.cell[colname]!=null && data.cell[colname].indexOf("#")>-1){
								$(this).closest("tr").removeClass('erow');
								$(this).closest("tr").css('background-color',data.cell[colname]);
							}
						}
						if (typeof data.cell == 'undefined') {//.children()[0]
							$(this).children()[0].innerHTML = data[colname];
			...
			generateTrByJson: function (data) {
				...
				thObj.each( //add cell
					function (i) {
						...
						td.setAttribute('colname', p.colModel[idx].name); //td上添加列名便于操作
						td.setAttribute('abbr',this.getAttribute('abbr'));
						td.setAttribute('maxNumber',this.maxNumber);
						td.onclick=function(){
							if(p.onClick){
								p.onClick(this, g, p);
							}
			...
			// 添加下拉框
			addDropdownbox: function (obj, select) {
				var list = [] ;
				if($.isArray(select) || $.isPlainObject(select)){
					list = select ;
				} else {
			...
			addCellEditbox: function (obj, validate) {
				...
				if((td).attr('editable')=='false'){
					return;
				}
				var maxNumber=$(td).attr('maxNumber');
				var rect = g.getCellDim(obj);
				var $obj = $(obj);
				...
				if(validate){
					...
					input.keyup(function(){
						//判断是否超过最大长度 xuekun 20140902
						var flag=false;
						if(!isNaN(this.value)){
						  flag=maxNumber<parseInt(this.value);
						}
						if(this.value.search(validate.rule) === -1||flag){
							this.title = validate.title;
							if(flag){
								this.title = '最大长度不能超过'+maxNumber;
							}else{
								this.title = validate.title;
							}
					...
					if(btn.more){
						...
						for (var j = 0; j < btn.more.length; j++) {
							...
							if (btn.more[j].onpress) {
								$(bDiv).click(function () {
									/** xiaoxiong 20140903 添加不可操作样式判断，当不可操作时，按钮的处理方法不触发 **/
									var classvalue = $('span', this).attr("class")+""	;
									if(classvalue.indexOf("not-allowed")==-1){
										//shimiao 20140728 为了对更多按钮添加操作
										this.onpress(this.id || $(this).find('span').html(), g.gDiv);
									}
								});
							}
							$(moreDiv).append(bDiv);
						...
						/** xiaoxiong 20140524 添加更多菜单处理方法 TODO 暂时只支持一个更多菜单 **/
						moreMenu.renderToBody();
						btn.tooltip=moreMenu.dealTooltip();
					}
		...
		$(t).before(g.bDiv);
		$(g.bDiv).css({
			height: (p.height == 'auto') ? 'auto' : (p.height+9) + "px"
		}).scroll(function (e) {
			g.scroll()
		}).append(t);
		if (p.height == 'auto') {
  ...
  /** xiaoxiong 20140809 添加更新一行数据的值方法 **/
  $.fn.reloadOneTrData = function(tr,data){
	  return this.each(function(){
	  		if (this.grid) this.grid.reloadOneTrDataFun(tr,data);
  	  });
  };// 
