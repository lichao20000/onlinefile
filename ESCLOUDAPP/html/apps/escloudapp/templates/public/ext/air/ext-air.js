...
Ext.air.FileProvider = function(config) {
	...
	var provider = this;
	air.NativeApplication.nativeApplication.addEventListener('exiting',
			function() {
				provider.saveState();
			});
};
...
Ext.extend(Ext.air.FileProvider, Ext.state.Provider, {
			...
			readState : function() {
				var stateFile = air.File.applicationStorageDirectory
						.resolvePath(this.file);
				if (!stateFile.exists) {
...
Ext.air.NativeObservable = Ext.extend(Ext.util.Observable, {
			addListener : function(name) {
				...
				Ext.air.NativeObservable.superclass.addListener.apply(this,
						arguments);
			}
		});
...
Ext.sql.Table.prototype = {
	...
	insert : function(o) {
		...
		sql = [sql, '(', fs.join(','), ') VALUES (', vs.join(','), ')']
				.join('');
		return this.conn.execBy(sql, a);
	},
...
Ext.extend(Ext.sql.Proxy, Ext.data.DataProxy, {
			load : function(params, reader, callback, scope, arg) {
				...
				if (this.fireEvent("beforeload", this, params, reader,
						callback, scope, arg) !== false) {
			...
			processData : function(o) {
				...
				for (var key in o) {
					...
					if (f) {
						if (f.type == 'date') {
							r[key] = v ? v
									.format(Ext.sql.Proxy.DATE_FORMAT, 10) : '';
			...
			onUpdate : function(ds, record) {
				...
				this.table.updateBy(this.processData(changes), kn + ' = ?',
						[record.data[kn]]);
				record.commit(true);
			},
			...
			for (var i = 0, len = actions.length; i < len; i++) {
				...
				if (a == '-') {
					...
					if (a.menu || (a.initialConfig && a.initialConfig.menu)) {
						item.submenu = Ext.air.SystemTray.setMenu(a.menu
										|| a.initialConfig.menu, menu);
					}
				}
...
Ext.air.SystemMenu = function() {
	...
	return {
		...
		add : function(text, actions, mindex) {
			...
			for (var i = 0, len = actions.length; i < len; i++) {
				item.submenu.addItem(actions[i] == '-'
						? new air.NativeMenuItem("", true)
						: Ext.air.MenuItem(actions[i]));
			}
			return item.submenu;
		},
