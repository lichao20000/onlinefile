 ...
 * 
 * Wraps the AIR NativeWindow class to give an Ext friendly API. <br/><br/>This
 * class also adds automatic state management (position and size) for the window
 * (by id) and it can be used for easily creating "minimize to system tray" for
 * the main window in your application.<br/><br/>
 * 
 * Note: Many of the config options for this class can only be applied to NEW
 * windows. Passing in an existing instance of a window along with those config
...
Ext.air.NativeWindow = function(config) {
	...
	if (!this.instance) {
		...
		this.loader = window.runtime.flash.html.HTMLLoader.createRootWindow(
				false, options, false);
	...
	var x = !Ext.isEmpty(state.x, false) ? state.x : (!Ext.isEmpty(this.x,
			false) ? this.x : centerX);
	var y = !Ext.isEmpty(state.y, false) ? state.y : (!Ext.isEmpty(this.y,
			false) ? this.y : centerY);
	...
	win.addEventListener('move', function() {
				if (win.displayState != air.NativeWindowDisplayState.MINIMIZED
						&& win.width > 100 && win.height > 100) {
					state.x = win.x;
					state.y = win.y;
				}
			});
	...
	/**
	 * @cfg {Boolean} minimizeToTray True to enable minimizing to the system
	 *      tray. Note: this should only be applied to the primary window in
	 *      your application. A trayIcon is required.
	 */
...
Ext.extend(Ext.air.NativeWindow, Ext.air.NativeObservable, {
	...
	/**
	 * @cfg {air.NativeWindow} instance The native window instance to wrap. If
	 *      undefined, a new window will be created.
	 */

	/**
	 * @cfg {String} trayIcon The icon to display when minimized in the system
	 *      tray
	 */
	/**
	 * @cfg {NativeMenu} trayMenu Menu to display when the tray icon is right
	 *      clicked
	 */
	/**
	 * @cfg {String} trayTip Tooltip for the tray icon
	 */

	/**
	 * @cfg {String} chrome The native window chrome (defaults to 'standard',
	 *      can also be 'none').
	 */
	chrome : 'standard', // can also be none
	/**
	 * @cfg {String} type The native window type - normal, utility or
	 *      lightweight. (defaults to normal)
	 */
...
/**
 * Returns the javascript "window" object of the first opened window in your
 * application
 * 
 * @return Window
 * @static
...
Ext.air.NativeWindowGroup = function() {
	...
	return {
		...
		/**
		 * Executes the specified function once for every window in the group,
		 * passing each window as the only parameter. Returning false from the
