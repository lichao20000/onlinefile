...
(function() {
	...
	El.prototype = {
		...
		/** xiaoxiong 20140528 暂时将IE与其他的获取ID的方式修改为一样的 **/
...
//		},
	...
	El.garbageCollect = function(cleanFly) {
...
Ext.override(Date, {
	...
	getWeekOfYear : function() {
		var ms1d = 864e5;
		var ms7d = 7 * ms1d;
		var DC3 = Date.UTC(this.getFullYear(), this.getMonth(), this.getDate()
						+ 3)
				/ ms1d;
...
Ext.util.Format = function() {
	...
	return {
		...
		//jiangjien 20110822 add
		cnFloat : function(v,leng) {
			//alert(typeof v == "string");
			var tv = Number(v);
			var val =  String(tv);
			var ps = val.split('.');
			var whole = (ps[0]==''?'0':ps[0]);
			var t = ps[1] ?ps[1].length:0;
			var tempVal = "";
			for(var i=0;i<leng-t;i++){
				tempVal= tempVal+"0";
			}
			//var sub = ps[1] ? '.' + ps[1] : "."+tempVal;
			var sub = (ps[1]?ps[1]:"") +tempVal;
			val = whole + "."+sub;
			return val;
		},
...
Ext.extend(Ext.data.Store, Ext.util.Observable, {
			...
			/* niuhe 20130703 add insertWithSnapShot function to insert data to store and update snapshot parameters */
			insertWithSnapShot : function(index, records) {
				records = [].concat(records);
				for (var i = 0, len = records.length; i < len; i++) {
					this.data.insert(index, records[i]);
					if(this.snapshot){
			...
			load : function(options) {
				...
				if (this.fireEvent("beforeload", this, options) !== false) {
					// huying 20100529 如果store有PagingToolbar的引用并且此PagingToolbar带有ComboBox则将limit参数直接取ComboBox的value
					if(this.PagingToolbar && this.PagingToolbar.ComboBox){
						// huying 20100602 修复代码漏洞，如果options不传或options.params不传会报js错误
						// huying 20110623 
						options.params = options.params || {start: 0} ;
						if(options && options.params){
							options.params.limit = this.PagingToolbar.ComboBox.getValue()==''?-1:this.PagingToolbar.ComboBox.getValue() ;
						}
					}
					// end huying
			...
			loadRecords : function(o, options, success) {
			   try{ //liukaiyuan  add try catch
				...
				//liukaiyuan 20101117 add currPage for keyword search
				var r = o.records, t = o.totalRecords || r.length ,tc = o.thisCurrPage;
			...
			sortData : function(f, direction) {
				...
				var fn = function(r1, r2) {
					...
					if(typeof(v1) == 'string'){//chenzhenhai 20120427 按系统缺省区域排序

							var pattern=/[^\x00-\xff]/g;//fuzhilin 20120924 判断是否包含中文
							var reg = /[^u4e00-u9fa5]/;//fuzhilin 20120924 判断该字符是否是汉字
							if(pattern.test(v1)&&pattern.test(v2)){//fuzhilin 20120924 判断包含中文时的排列顺序
									var v1length=v1.length ;
									var v2length=v2.length ;
									var mn=0;	
									if(v1length>=v2length){
										
											for(var i=0;i<=v2length;i++){
												if(v1.substring(i,i+1).charCodeAt(0)>=128&&v2.substring(i,i+1).charCodeAt(0)>=128){
													
															mn= v1.substring(i,i+1).localeCompare(v2.substring(i,i+1))
															if(mn!=0){
																return mn ;
															}
															
												}else{
														mn= v1.substring(i,i+1) > v2.substring(i,i+1) ? 1 : (v1.substring(i,i+1)< v2.substring(i,i+1) ? -1 : 0)
															if(mn!=0){
																return mn ;
															}
												}
								...
								if(v1==''||v2==''){//fuzhilin 20120924 增加空格判断
									if(v1.localeCompare(v2)!=0){
										mn=v1.localeCompare(v2)
											if(mn==1){
												return -1
											}else{
												return 1;
...
Ext.extend(Ext.data.JsonReader, Ext.data.DataReader, {
	...
	readRecords : function(o) {
		...
		if (!this.ef) {
			...
			//liukaiyuan 20101117 add currPage for keyword search
			if (s.currPage) {
				this.getCurrPage = this.getJsonAccessor(s.currPage);
			}
		...
		var root = this.getRoot(o), c = root.length, totalRecords = c, success = true ,thisCurrPage = -1;
		if (s.totalProperty) {
		...
		if (s.currPage) {
			var v = parseInt(this.getCurrPage(o), 10);
			if (!isNaN(v)) {
				thisCurrPage = v;
			}
		}
...
Ext.extend(Ext.data.Node, Ext.util.Observable, {
	...
	/** niuhe 20130524 数据授权-用于在目录树叶子节点未展开(已加载)时，获取各个叶子节点的勾选状态。
		使用该方法的前提是目录树是一次加载全部数据，而并非异步加载。原方法参见：cascade **/
	cascadeChildNodes : function(fn, scope, args) {
		if (fn.apply(scope || this, args || [this]) !== false) {
			var cs = [];
			if(this.attributes.children){
				var cs = this.attributes.children;
				for (var i = 0, len = cs.length; i < len; i++) {
					this.getLoader().createNode(cs[i]).cascadeChildNodes(fn, scope, args);
				}	
			}else if(this.childNodes){
				var cs = this.childNodes;
				for (var i = 0, len = cs.length; i < len; i++) {
					cs[i].cascadeChildNodes(fn, scope, args);
				}
			}
		}
	},
...
Ext.Container = Ext.extend(Ext.BoxComponent, {
			...
			add : function(comp) {
				...
				if (this.fireEvent('beforeadd', this, c, pos) !== false
					...
					this.items.add(c);
					c.ownerCt = this;
					this.onAdd(c);// liukaiyuan 20091021 add
			...
			remove : function(comp, autoDestroy) {
				...
				if (c && this.fireEvent('beforeremove', this, c) !== false) {
					...
					this.onRemove(c);// liukaiyuan 20091021 add
		...
		if (!this.collapsedEl) {
			...
			if (this.collapseMode == 'mini') {
				...
				if (this.floatable !== false) {
					this.collapsedEl.addClassOnOver("x-layout-collapsed-over");
					this.collapsedEl.on("click", this.collapseClick, this);//xiaoxiong 20121017 open this line
					//this.collapsedEl.on('click', this.onExpandClick, this, {//xiaoxiong 20100430 modify collapsible bug
					//		stopEvent : true
					//	});
				}
			}
...
Ext.Window = Ext.extend(Ext.Panel, {
	...
	initHidden : true,
	monitorResize : true,
	shadow : false,/** xiaoxiong 20130522 去除窗体背景 **/
	frame : true,
	floating : true,
	initComponent : function() {
...
//		}
		...
		//liukaiyuan  20100323 ȫ�����Ϲرհ�ť ȡ�� 20100426
	...
	show : function(animateTarget, cb, scope) {
		...
		// liukaiyuan 20100323 ������ڸ߶ȴ�������߶�ʱ�����һ��
		var desktopEl = Ext.get('x-desktop');
		//xiaoxiong 20111020 添加判断 3D库房要是打开状态 将其最小化 并且当是消息窗体时 不用最小化
		if(this.getId() != 'RepositoryView_win' && Ext.getCmp('RepositoryView_win')){
			if(this.getId().substring(0 , 4) != 'Msg_'){
				Ext.getCmp('RepositoryView_win').hide();
			}
		}
		//end
		var isWin = Ext.get('ux-taskbar');
		var isHeight = false;
		var isWidth = false;
		if (isWin) {
			if (this.getSize().height > desktopEl.getHeight()) {
				this.setHeight(desktopEl.getHeight());
				isHeight = true;

			}
			//xiaoxiong 20100823 add
			if (this.getSize().width > desktopEl.getWidth()) {
				this.setWidth(desktopEl.getWidth());
				isWidth = true;

			}
			//end
		} else {
			if (this.getSize().height > document.body.clientHeight) {
				this.setHeight(document.body.clientHeight);
				isHeight = true;
			}
			//xiaoxiong 20100823 add
			if (this.getSize().width > document.body.clientWidth) {
...
Ext.DatePicker = Ext.extend(Ext.Component, {
	...
	onRender : function(container, position) {
		...
		this.rightcr = new Ext.util.ClickRepeater(this.el
	...
	onDestroy : function() {
		...
		if (this.rendered) {
			Ext.destroy(this.leftcr, this.rightcr, this.monthPicker,
					this.eventEl);
		}
	},
	beforeDestroy : function() {
...
Ext.TabPanel = Ext.extend(Ext.Panel, {
	...
	initEvents : function() {
		...
		this.on('add', this.onAddTab, this);// liukaiyuan
		// this.on('add', this.onAdd, this);
		// this.on('remove', this.onRemove, this);
		this.on('remove', this.onRemoveTab, this);
	...
	// liukaiyuan 20091021 change the method name
	onAddTab : function(tp, item, index) {
...
Ext.Button = Ext.extend(Ext.Component, {
	...
	aotoShowMenu : true,/** xiaoxiong 20130511 添加自动显示下拉菜单 **/
	tooltipType : 'qtip',
	...
	onMouseOver : function(e) {
		/** xiaoxiong 20130511 添加自动显示菜单属性设置aotoShowMenu **/
		if (this.aotoShowMenu && this.menu && !this.menu.isVisible() && !this.ignoreNextClick && !this.disabled) {
				this.showMenu();
		}
		if (!this.disabled) {
...
/** xiaoxiong 20121025 添加changePageAction 和 allDataCount 为了标识store刷新时是分页 为了不再计算总数 提高翻页性能 **/
Ext.PagingToolbar = Ext.extend(Ext.Toolbar, {
	...
	lastText : "Last Page",
	refreshText : "Refresh",
	changePageAction : false,
	// huying 20100613 重新添加属性用于有的地方要排除分页条目显示选择
	withComBox : true,
	withComBoxMaxCount : 100,
	paramNames : {
		start : 'start',
		limit : 'limit',
	...
	//add by haojianke on 20100602 Records of current to load the data start position  --begin
	setPageSize : function( curPageSize )
	{
	...
	{
		return this.getPageData().activePage ;
	},
	gotoPage : function( curPageSize )
	{
		this.field.dom.value = curPageSize ;
		this.doLoad( ( parseInt( curPageSize ) - 1 ) * this.pageSize ) ;
	},
	//--end haojianke 20100602
	onRender : function(ct, position) {
		...
		this.afterTextEl = this.addText(String.format(this.afterPageText, 0));
...
// huying 20100529 修复分页有不带ComboBox的bug
// huying 20100613 通过初试化item和withComBox属性共同决定是否带分页条目显示选择器
		if(!this.initialConfig.items && this.withComBox){
			var obj = this ;
			var data = [[10,10], [20,20],[50,50],[100,100]] ;
			if(this.withComBoxMaxCount>100){
				data = [[10,10], [20,20],[50,50],[100,100],[this.withComBoxMaxCount,this.withComBoxMaxCount]] ;
			}
			this.ComboBox = new Ext.form.ComboBox(
				{
					triggerAction:'all',
					store:new Ext.data.SimpleStore({fields:['value','display'],data:data}),
					...
					listeners:{select: function(){
						...
						obj.store.load({params: {start: 0, limit: this.value,changePageAction:true,allDataCount:obj.store.getTotalCount()}}) ;
					},valid: function(){
						obj.pageSize = this.value ;
					}}
				}
	...
	onLoad : function(store, r, o) {
		...
		this.first.setDisabled(ap == 1 || ap == 0);//xiaoxiong 20100430 modify changePage bug
		this.prev.setDisabled(ap == 1 || ap == 0);
	...
	getPageData : function() {
		...
		// shilongfei 20110105 进行相关操作(如删除)后，导致当前页无数据并且当前页码大于总页数，则跳转到最后页 add
		var activePage = 0;
		if (total > 0){
			var maxPage = Math.ceil(total / this.pageSize);
		...
		// shilongfei 20110105 end
		return {
			total : total,
			// shilongfei 20110105 同上 add
//			activePage : Math.ceil(total > 0 ? (this.cursor + this.pageSize)
//					/ this.pageSize : 0),
			activePage:activePage,
			// shilongfei 20110105 end
			pages : total < this.pageSize ? (Math.ceil(total > 0
					? (this.cursor + this.pageSize) / this.pageSize
					: 0)) : Math.ceil(total / this.pageSize)
		};
	},
	...
	beforeLoad : function() {
	//jiangyuntao 20110829 edit 如果为空store 直接返回
	    if(!this.store.proxy)return;
	...
	doLoad : function(start) {
		...
		o[pn.changePageAction] = this.changePageAction;
		o[pn.allDataCount] = this.store.getTotalCount();
		this.changePageAction = false;
	...
	bind : function(store) {
		store = Ext.StoreMgr.lookup(store);
		// huying 20100529 将PagingToolbar对象引用赋给PagingToolbar所绑定的stroe
		store.PagingToolbar = this ;
...
Ext.MessageBox = function() {
	...
	var handleButton = function(button) {
		if (dlg.isVisible()) {
			/** xiaoxiong 20130622 添加不需要关闭提示窗体配置 **/
			if(dlg.noAutoHodeButton != button){
				dlg.hide();
			}
	...
	return {
		getDialog : function(titleText) {
			if (!dlg) {
				dlg = new Ext.Window({
							...
							buttonAlign : "center",
							/** xiaoxiong 20130622 添加不需要关闭提示窗体配置 **/
							noAutoHodeButton : opt.noAutoHodeButton,
							width : 400,
							height : 100,
							minHeight : 80,
							plain : true,
		...
		show : function(options) {
			...
			/** xiaoxiong 20130622 添加不需要关闭提示窗体配置 **/
			d.noAutoHodeButton=opt.noAutoHodeButton;
		...
		alert : function(title, msg, fn, scope) {
			this.show({
						title : title,
						msg : msg,
						minWidth : 150,//liukaiyuan 20100406
						buttons : this.OK,
						fn : fn,
						scope : scope
					});
		...
		confirm : function(title, msg, fn, scope) {
			this.show({
						title : title,
						msg : msg,
						minWidth : 200,//liukaiyuan 20100524
						buttons : this.YESNO,
						fn : fn,
						scope : scope,
						icon : this.QUESTION
...
Ext.tree.TreePanel = Ext.extend(Ext.Panel, {
			...
			/** niuhe 20130524 数据授权-用于在目录树叶子节点未展开(已加载)时，获取各个叶子节点的勾选状态。
				使用该方法的前提是目录树是一次加载全部数据，而并非异步加载。原方法参见：getChecked **/
			getCheckedCascadeChildNodes : function(a, startNode) {
				startNode = startNode || this.root;
				var r = [];
				var f = function() {
					if (this.attributes.checked) {
...
Ext.tree.TreeNode = function(attributes) {
	...
	this.rendered = false;
	this.id_structure = attributes.id_structure ;
	...
	expand : function(deep, anim, callback) {
		if (!this.expanded) {
			...
			if (!this.isHiddenRoot()
					&& (this.getOwnerTree() && this.getOwnerTree().animate && anim !== false)//添加this.getOwnerTree()是否存在的判断 niuhe 20130409 
					|| anim) {
...
Ext.menu.Menu = function(config) {
	...
	this.addEvents('beforeshow', 'beforehide', 'show', 'hide', 'click',
			'mouseover', 'mouseout', 'itemclick', 'contextmenu'); // liukaiyuan
	// add
	// 'contextmenu'
	// 20091014
...
Ext.extend(Ext.menu.Menu, Ext.util.Observable, {
			...
			render : function() {
				...
				ul.on("contextmenu", this.onContextmenu, this);// liukaiyuan
				// 20091014 add
				this.items.each(function(item) {
...
Ext.form.TextField = Ext.extend(Ext.form.Field, {
	...
	autoValidate : false , // huying 20100518 can choose
	ifSingleQuotesAndSlash:true,//yuzhijia 20110623 add 西工大项目要求档案著入能输入单引号，反斜杠，
	                            //为了不影响其他地方的验证，故此添加此标记，默认为验证的，不验证设为false 即可
	initComponent : function() {
	...
	validateValue : function(value) {
		//huying 20100518  change value to byte tvalue
		var tvalue = value.trim().replace(/[^\x00-\xff]/g, '**') ;
		// huying 20111017
		if(this.inputType == 'file' || this.inputType == 'password'){
			tvalue = value ;
		}
		if (tvalue.length < 1 || tvalue === this.emptyText) {
			if (this.allowBlank) {
				this.clearInvalid();
				return true;
			} else {
		...
		if (tvalue.length < this.minLength) {
			/**this.markInvalid(String.format(this.minLengthText, this.minLength));**/
			/**zhengfang 20130529 修改文本框minLength验证失败的提示信息，改为该输入项的最小长度{0}个汉字**/
			this.markInvalid(String.format(this.minLengthText, parseInt(this.minLength/2)));
			return false;
		}
		if (tvalue.length > this.maxLength) {
			/**this.markInvalid(String.format(this.maxLengthText, this.maxLength));**/
			/**zhengfang 20130529 修改文本框minLength验证失败的提示信息，改为该输入项的长度限制在{0}个汉字以内**/
			this.markInvalid(String.format(this.maxLengthText, parseInt(this.maxLength/2)));
			return false;
		}
		if (this.vtype) {
		...
		// huying 20100518 validate special char//yuzhijia 20110623 add ifSingleQuotesAndSlash 西工大项目要求档案注入不验证单引号，反斜杠
		if(this.autoValidate && this.inputType != 'file' && this.inputType != 'password' && this.ifSingleQuotesAndSlash){
			if(/['"\\]/gi.test(value)){
				this.markInvalid('不能包含英文单引号、双引号和反斜杠！');
				return false;
			//xiaoxiong 20120213 添加 对‘<?’和‘<!’等注释字符串 的过滤 避免出现问题
			} else if(value.indexOf('<?')>-1 || value.indexOf('<!')>-1 || value.indexOf('<%')>-1 ){
				this.markInvalid('不能包含‘&lt;?’、‘&lt;!’、‘&lt;%’等注释字符串！');
				return false;
			}
		}
	...
	/* niuhe 20130729 根据validateValue得到仅用于DataField的验证 */
	/* 此方法的regex验证优先于length的验证，其他部分与validateValue方法保持一致 */
	validateValueForDataField : function(value) {
		var tvalue = value.trim().replace(/[^\x00-\xff]/g, '**') ;
...
/** xiaoxiong 20121128 不支持回车的TEXTAREA 同于档案著录 相当于支持换行的textField组建 **/
Ext.form.TextSimpleArea = Ext.extend(Ext.form.TextField, {
			growMin : 60,
			growMax : 1000,
			growAppend : '&#160;\n&#160;',
			growPad : 0,
			...
			/** xiaoxiong 20130304 注释掉Ext.form.TextSimpleArea组建的onKeyUp个性化处理事件 BUG:6911
			onKeyUp : function(e) {
				if (!e.isNavKeyPress() || e.getKey() == e.ENTER) {
					event.keyCode = 9 ;
				}
...
Ext.reg('textsimplearea', Ext.form.TextSimpleArea);	//shimiao 20130329 绑定类型名。
/* shilongfei 20110105 
	修改NumberField的属性allowDecimals默认值为false，使其默认情况下仅支持整数的输入。
	如需支持实数的输入，即小数，修改其属性allowDecimals为true即可
*/
Ext.form.NumberField = Ext.extend(Ext.form.TextField, {
	fieldClass : "x-form-field x-form-num-field",
	allowDecimals : true,
	autoParseFloat : true,//xiaoxiong 20111018 
	autoFillZero : true,//xiaoxiong 20120924 自动补零功能是否放开
	decimalSeparator : ".",
	decimalPrecision : 2,
	allowNegative : true,
	...
	/**zhengfang 20130707 继承TextField属性，方便判断输入长度 start**/
	minLength : 0,
	maxLength : Number.MAX_VALUE,
	minLengthText : "The minimum length for this field is {0}",
	maxLengthText : "The maximum length for this field is {0}",
	/**zhengfang 20130707 继承TextField属性，方便判断输入长度 end**/
	...
	superValidateValue : function(value) {
		var tvalue = value.trim().replace(/[^\x00-\xff]/g, '**') ;
		if(this.inputType == 'file' || this.inputType == 'password'){
			tvalue = value ;
		}
	...
	validateValue : function(value) {
		...
		/*jiangyuntao 20120214 edit 
		 * 允许小数的NumberField，
		 * 在失去焦点时如果是以小数点结束则自动在最后补一个零,
		 * 如果是以小数点开台则自动在第一位补一个零。
		 * 手动监听blur事件，覆写处理函数后此功能失效。*/
		if(this.allowDecimals && this.autoFillZero && value && String(value).substring(value.length-1)==='.'){
			this.on('blur',function(f){
				var v = f.getRawValue();
				if(v.length-v.replace(/\./g,'').length == 1){
					if(v.substring(v.length-1)==='.'){
						this.setRawValue(v+'0');
						this.validate();
					}else if(v.substring(0,1)==='.'){
						this.setRawValue('0'+v);
						this.validate();
					}
				}
			});
		...
		/**zhengfang 20130707 数值类型字段长度校验和提示 start**/
		if(!(this.allowDecimals)){
		...
		/**zhengfang 20130707 数值类型字段长度校验和提示 end**/
		/**
		if (!Ext.form.NumberField.superclass.validateValue.call(this, value)) {
			return false;
		}
		**/
		if (!this.superValidateValue(value)) {
			return false;
		}
		if (value.length < 1) {
			return true;
...
Ext.form.DateField = Ext.extend(Ext.form.TriggerField, {
	...
	noShow:false, //jiangyuntao 20110924 增加支持noshow属性 为true不允许选择 默认可以选择
	...
	*/
	validateValue : function(value) {
		...
		/**zhengfang 20130708 日期类型的输入超长时直接提示正确的格式输入 start**/
		/* niuhe 20130729 日期类型的输入超长时直接提示正确的格式输入 */
		if (!Ext.form.DateField.superclass.validateValueForDataField.call(this, value)) {
			return false;
		}
		/*
		...
		*/
		/**zhengfang 20130708 日期类型的输入超长时直接提示正确的格式输入 end**/
		if (value.length < 1) {
			return true;
		}
		var svalue = value;
	...
	//niuhe 20130712 时间类型组件的验证
	/* niuhe 20130729 注释掉 */
	/*
	validateTimeFieldValue : function(value) {
		value = this.formatDate(value);
		if (!this.superValidateValue(value)) {
			return false;
		}
	...
	*/
	...
	//jiangyuntao 20110924 增加支持noshow属性 为true不允许选择
		if (this.disabled || this.noShow || this.readOnly) {
			return;
		}
		if (this.menu == null) {
	...
	beforeBlur : function() {
		//jiangjien 20110908 note,Because this.parseDate() has bug.
...
Ext.form.ComboBox = Ext.extend(Ext.form.TriggerField, {
			...
			minChars : 4,
			typeAhead : false,
			noShow : false,    //jiangyuntao 20110819 edit 增加不允许选择的属性控制
			queryDelay : 500,
			pageSize : 0,
			selectOnFocus : false,
			queryParam : 'query',
			...
			expand : function() {
			//jiangyuntao 20110819 edit 通过扩展的noShow属性控制是否允许下拉
				//if (this.isExpanded() || !this.hasFocus)
				if (this.isExpanded() || !this.hasFocus || this.noShow || this.readOnly) {
					return;
				}
				this.list.alignTo(this.wrap, this.listAlign);
				this.list.show();
...
Ext.extend(Ext.form.BasicForm, Ext.util.Observable, {
	timeout : 30,
	activeAction : null,
	//jiangyuntao 20110810 edit 增加表单提交时是否验证属性  默认为验证。
	isDoValid : true,
	...
	submit : function(options) {
		if (this.standardSubmit) {
			var v = this.isValid();
			//var v = true ;
			//jiangyuntao 20110810 edit 增加表单提交时是否验证属性
			if (v || this.form.isDoValid === false) {
				this.el.dom.submit();
			}
			return v;
		}
...
Ext.FormPanel = Ext.extend(Ext.Panel, {
			...
			onRemove : function(c) {
				...
				if (this.isField(c)) {
					...
					Ext.each(c.findBy(this.isField), this.form.remove,
							this.form);
				}
				}catch(e){};
			}
...
Ext.form.HtmlEditor = Ext.extend(Ext.form.Field, {
	enableFormat : true,
	enableFontSize : true,
	enableBackColors:true,//tangshuang add 20100512 ������ʽ������ɫ��
	enableColors : true,
	...
	createToolbar : function(editor) {
		...
		//tangshuang 20100512 add
		if(this.enableBackColors){
			tb.add('-', {
						itemId : 'backcolor',
	...
	initEditor : function() {
		...
		Ext.EventManager.on(this.doc, {
					...
					'keydown' : this.onKeyDown,/**fuhongyi 20100818 add onKeyDown*/
					buffer : 100,
					scope : this
				});
		if (Ext.isGecko) {
	...
	/**fuhongyi 20100818 add onKeyDown*/
	onKeyDown : function(e) {
...
(function() {
	...
	execCmd : function(cmd, value) {
		try{this.doc.execCommand(cmd, false, value === undefined ? null : value);
		this.syncValue();}catch(e){}
	},
	...
	//fuhongyi add title and text show cn. 20100329
	buttonTips : {
		bold : {
//			title : 'Bold (Ctrl+B)',
...
Ext.form.TimeField = Ext.extend(Ext.form.ComboBox, {
	...
	*/
	...
	/**zhengfang 20130708 时间类型的输入超长时直接提示正确的格式输入 start**/
	/* niuhe 20130729 还原validateValue */
	...
	validateValue : function(value) {
		...
		//niuhe 20130712 change validateValue() to validateTimeFieldValue() 
		//if (!Ext.form.DateField.prototype.validateValue(value)) {
		if (!Ext.form.DateField.prototype.validateTimeFieldValue(value)) {
			return false;
		}
		return true;
	},
...
Ext.extend(Ext.form.Action.Submit, Ext.form.Action, {
			...
			run : function() {
				...
				//jiangyuntao 20110810 edit 增加表单提交时是否验证属性
				if (o.clientValidation === false || this.form.isValid() || this.form.isDoValid === false) {
...
Ext.form.VTypes = function() {
	...
	return {
		...
		'repetition' : function(v) {
			return repetition.test(v);
		},
		// 100 lirong 20100120
		'ip' : function(v) {
...
//xiaoxiong 20130129 自定义扩展：将Grid表头中的全选复选框取消复选
...
Ext.extend(Ext.grid.GridView, Ext.util.Observable, {
	...
	layout : function() {
		...
		if (g.autoHeight) {
			...
			if (this.innerHd) {
				...
				//xiaoxiong 20111013 处理gridpanel出现右侧滚动条时，向右拖动滚动条到最右侧时，列标题和内容错位BUG
				this.innerHd.style.width = (vw - 18) + 'px';
			}
		}
		if (this.forceFit) {
	...
	focusCell : function(row, col, hscroll) {
		...
		this.focusEl.setXY(this.scroller.getXY() || xy);
		if (Ext.isGecko) {
			this.focusEl.focus();
		} else {
	...
	ensureVisible : function(row, col, hscroll) {
		...
		return cellEl ? Ext.fly(cellEl).getXY() : [
...
//						if (e.button === 0 && !e.shiftKey && !e.ctrlKey) {
				//fuhongyi 20100529 edit checkbox remaining selected 
				   var target = e.getTarget();
	...
	handleMouseDown : function(g, rowIndex, e) {//alert(333555);
...
Ext.grid.EditorGridPanel = Ext.extend(Ext.grid.GridPanel, {
	...
	onEditComplete : function(ed, value, startValue) {
		//jiangjien add 20110923
		if(ed && ed.field && ed.field instanceof Ext.form.DateField){
		}else{
			// huying 20110914 
			if (String(value) == String(startValue)) {
				if(ed && ed.field && ed.field['getRawValue']){
					value = ed.field.getRawValue() ;
				}
			}
		}
		// huying 20110914 end
		...
		startValue = this.postEditValue(startValue, startValue, r, field);/** niuhe 20130604 autoEncode为true时，value和startValue都需要转码后再进行比较 **/
		if (String(value) !== String(startValue)) {
			var e = {
				grid : this,
				record : r,
...
Ext.grid.PropertyColumnModel = function(grid, store) {
	...
	this.editors = {
		...
		'string' : new g.GridEditor(new f.TextField({
					/**chenjian 20130313 add 缺陷ID：[6934] 限制组件名称不为空*/
					allowBlank:false,
					emptyText:'请输入...',
					blankText:'这个字段不能为空',
					selectOnFocus : true,
					maxLength  : 50//chenjian 20120912 缺陷ID：[6126] 限制字段名称的长度
				})),
		'number' : new g.GridEditor(new f.NumberField({
					...
					maxValue:4000
					,allowDecimals : false//xiaoxiong 20111019 修改表单编辑界面 编辑组件属性时 数值型字段后边自动加小数点问题
				})),
...
Ext.Slider.Vertical = {
	...
	onClickChange : function(local) {
		if (local.left > this.clickRange[0] && local.left < this.clickRange[1]) {
			...
			this
					.setValue(Math.round(bottom / this.getRatio()), undefined,
							true);
		}
	}
};
...
//fuhongyi 20100529 add for archive EditGrid ,if records selected but Can't be cancel (bug) 
