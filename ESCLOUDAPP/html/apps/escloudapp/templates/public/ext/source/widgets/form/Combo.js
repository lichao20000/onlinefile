...
Ext.form.ComboBox = Ext.extend(Ext.form.TriggerField, {
	/**
	 * @cfg {Mixed} transform The id, DOM node or element of an existing HTML
	 *      SELECT to convert to a ComboBox. Note that if you specify this and
	 *      the combo is going to be in a {@link Ext.form.BasicForm} or
	 *      {@link Ext.form.FormPanel}, you must also set {@link #lazyRender} =
	 *      true.
	 */
	/**
	 ...
	 */
	/**
	 * @cfg {Ext.data.Store/Array} store The data source to which this combo is
	 *      bound (defaults to undefined). This can be any
	 *      {@link Ext.data.Store} subclass, a 1-dimensional array (e.g.,
	 *      ['Foo','Bar']) or a 2-dimensional array (e.g.,
	 *      [['f','Foo'],['b','Bar']]). Arrays will be converted to a
	 *      {@link Ext.data.SimpleStore} internally. 1-dimensional arrays will
	 *      automatically be expanded (each array item will be the combo value
	 *      and text) and for multi-dimensional arrays, the value in index 0 of
	 *      each item will be assumed to be the combo value, while the value at
	 *      index 1 is assumed to be the combo text.
	 */
	/**
	 ...
	 *      value to automatically post during a form submission. Note that the
	 *      hidden field's id will also default to this name if
	 *      {@link #hiddenId} is not specified. The combo's id and the hidden
	 *      field's ids should be different, since no two DOM nodes should share
	 *      the same id, so if the combo and hidden names are the same, you
	 *      should specify a unique hiddenId.
	 */
	/**
	 * @cfg {String} hiddenId If {@link #hiddenName} is specified, hiddenId can
	 *      also be provided to give the hidden field a unique id (defaults to
	 *      the hiddenName). The hiddenId and combo {@link #id} should be
	...
	maxHeight : 300,
	/**
	 * @cfg {Number} minHeight The minimum height in pixels of the dropdown list
	 *      when the list is constrained by its distance to the viewport edges
	 *      (defaults to 90)
	 */
	minHeight : 90,
	/**
	 ...
	 *      the text being typed after a configurable delay ({@link #typeAheadDelay})
	 ...
	 *      (defaults to undefined). If this defaut text is used, it means there
	...
	initList : function() {
		if (!this.list) {
			...
			if (!this.tpl) {
				 ...
				 *      custom XTemplate has been specified in {@link #tpl}
				 ...
				 *      determine what nodes the DataView which handles the
	 ...
	 * {@link #beforequery} event prior to performing the query allowing the
