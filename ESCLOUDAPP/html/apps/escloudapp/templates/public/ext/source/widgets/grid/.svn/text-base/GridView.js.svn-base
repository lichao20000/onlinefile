 ...
 *          {@link Ext.grid.GridPanel}. Methods of this class may be used to
 *          access user interface elements to enable special display effects. Do
 *          not change the DOM structure of the user interface.
 *          </p>
 *          <p>
 *          This class does not provide ways to manipulate the underlying data.
 *          The data model of a Grid is held in an {@link Ext.data.Store}.
 *          </p>
 * @constructor
 * @param {Object}
 *            config
...
Ext.grid.GridView = function(config) {
	...
	this.addEvents(
			/**
			 * @event beforerowremoved Internal UI Event. Fired before a row is
			 *        removed.
			...
			"beforerowremoved",
			/**
			 * @event beforerowsinserted Internal UI Event. Fired before rows
			 *        are inserted.
			 * @param {Ext.grid.GridView}
			 *            view
			 * @param {Number}
			 *            firstRow The index of the first row to be inserted.
			 * @param {Number}
			 *            lastRow The index of the last row to be inserted.
			 */
			"beforerowsinserted",
			/**
			 * @event beforerefresh Internal UI Event. Fired before the view is
			 *        refreshed.
			 * @param {Ext.grid.GridView}
			 *            view
			 */
			"beforerefresh",
			/**
			 * @event rowremoved Internal UI Event. Fired after a row is
			 *        removed.
			 * @param {Ext.grid.GridView}
			 *            view
			 * @param {Number}
			 *            rowIndex The index of the row that was removed.
			 * @param {Ext.data.Record}
			 *            record The Record that was removed
			 */
			"rowremoved",
			/**
			 * @event rowsinserted Internal UI Event. Fired after rows are
			 *        inserted.
			 * @param {Ext.grid.GridView}
			 *            view
			 * @param {Number}
			 *            firstRow The index of the first inserted.
			 * @param {Number}
			 *            lastRow The index of the last row inserted.
			 */
			"rowsinserted",
			/**
			 * @event rowupdated Internal UI Event. Fired after a row has been
			 *        updated.
			 * @param {Ext.grid.GridView}
			 *            view
			 * @param {Number}
			 *            firstRow The index of the row updated.
			 * @param {Ext.data.record}
			 *            record The Record backing the row updated.
			 */
			"rowupdated",
			/**
			 * @event refresh Internal UI Event. Fired after the GridView's body
			 *        has been refreshed.
			 * @param {Ext.grid.GridView}
			 *            view
			 */
...
Ext.extend(Ext.grid.GridView, Ext.util.Observable, {
	 ...
	 * rendering. You can also supply custom parameters to the row template for
	 * the current row to customize how it is rendered using the <b>rowParams</b>
	 * parameter. This function should return the CSS class name (or empty
	 * string '' for none) that will be added to the row's wrapping div. To
	 * apply multiple class names, simply return them space-delimited within the
	 * string (e.g., 'my-class another-class').
	 * 
	 * @param {Record}
	 *            record The {@link Ext.data.Record} corresponding to the
	 *            current row
	 * @param {Number}
	 *            index The row index
	 * @param {Object}
	 *            rowParams A config object that is passed to the row template
	 *            during rendering that allows customization of various aspects
	 *            of a body row, if applicable. Note that this object will only
	 *            be applied if {@link #enableRowBody} = true, otherwise it will
	 *            be ignored. The object may contain any of these properties:
	 *            <ul>
	 *            <li><code>body</code> : String <div class="sub-desc">An
	 *            HTML fragment to be rendered as the cell's body content
	 *            (defaults to '').</div></li>
	 *            <li><code>bodyStyle</code> : String <div class="sub-desc">A
	 *            CSS style string that will be applied to the row's TR style
	 *            attribute (defaults to '').</div></li>
	 *            <li><code>cols</code> : Number <div class="sub-desc">The
	 *            column count to apply to the body row's TD colspan attribute
	 *            (defaults to the current column count of the grid).</div></li>
	 *            </ul>
	 * @param {Store}
	 *            store The {@link Ext.data.Store} this grid is bound to
	 * @method getRowClass
	 * @return {String} a CSS class name to add to the row.
	 */
	/**
	 * @cfg {Boolean} enableRowBody True to add a second TR element per row that
	 *      can be used to provide a row body that spans beneath the data row.
	 *      Use the {@link #getRowClass} method's rowParams config to customize
	 *      the row body.
	 */
	/**
	 * @cfg {String} emptyText Default text to display in the grid body when no
	 *      rows are available (defaults to '').
	 */
	/**
	 * @property dragZone
	 * @type Ext.grid.GridDragZone
	 *       <p>
	 *       <b>This will only be present if the owning GridPanel was configured
	 *       with {@link Ext.grid.GridPanel#enableDragDrop enableDragDrop}<b>
	 *       <tt>true</tt></b>.
	 *       </p>
	 *       <p>
	 *       <b>This will only be present after the owning GridPanel has been
	 *       rendered</b>.
	 *       </p>
	 *       <p>
	 *       A customized implementation of a {@link Ext.dd.DragZone DragZone}
	 *       which provides default implementations of the template methods of
	 *       DragZone to enable dragging of the selected rows of a GridPanel.
	...
	scrollOffset : 19,
	/**
	 * @cfg {Boolean} autoFill True to auto expand the columns to fit the grid
	 *      <b>when the grid is created</b>.
	 */
	autoFill : false,
	/**
	 * @cfg {Boolean} forceFit True to auto expand/contract the size of the
	 *      columns to fit the grid width and prevent horizontal scrolling.
	 */
	forceFit : false,
	/**
	...
	/**
	 * @cfg {Number} cellSelectorDepth The number of levels to search for cells
	 *      in event delegation (defaults to 4)
	 */
	cellSelectorDepth : 4,
	/**
	 * @cfg {Number} rowSelectorDepth The number of levels to search for rows in
	 *      event delegation (defaults to 10)
	 */
	rowSelectorDepth : 10,
	...
	/**
	 * Return the &lt;TD> HtmlElement which represents the Grid's header cell
	 ...
	 * 
	 * @param {Boolean}
	 *            headersToo (optional) True to also refresh the headers
	 */
