...
/*
 * These classes are derivatives of the similarly named classes in the YUI
 * Library. The original license: Copyright (c) 2006, Yahoo! Inc. All rights
 * reserved. Code licensed under the BSD License:
...
(function() {
	...
	/**
	 * @class Ext.dd.DragDrop Defines the interface and base operation of items
	 *        that that can be dragged or can be drop targets. It was designed
	 *        to be extended, overriding the event handlers for startDrag,
	 *        onDrag, onDragOver and onDragOut. Up to three html elements can be
	 *        associated with a DragDrop instance:
	 *        <ul>
	 *        <li>linked element: the element that is passed into the
	 *        constructor. This is the element which defines the boundaries for
	 *        interaction with other DragDrop objects.</li>
	 *        <li>handle element(s): The drag operation only occurs if the
	 *        element that was clicked matches a handle element. By default this
	 *        is the linked element, but there are times that you will want only
	 *        a portion of the linked element to initiate the drag operation,
	 *        and the setHandleElId() method provides a way to define this.</li>
	 *        <li>drag element: this represents the element that would be moved
	 *        along with the cursor during a drag operation. By default, this is
	 *        the linked element itself as in {@link Ext.dd.DD}. setDragElId()
	 *        lets you define a separate element that would be moved, as in
	 *        {@link Ext.dd.DDProxy}. </li>
	 *        </ul>
	 *        This class should not be instantiated until the onload event to
	 *        ensure that the associated elements are available. The following
	 *        would define a DragDrop obj that would interact with any other
	 *        DragDrop obj in the "group1" group:
	 * 
	 * <pre>
	 * dd = new Ext.dd.DragDrop(&quot;div1&quot;, &quot;group1&quot;);
	 * </pre>
	 * 
	 * Since none of the event handlers have been implemented, nothing
	 *        would actually happen if you were to run the code above. Normally
	 *        you would override this class or one of the default
	 *        implementations, but you can also override the methods you want on
	...
	Ext.dd.DragDrop.prototype = {
		 ...
		 * this element. This lets us do things like only starting the drag when
		 * the header element within the linked html element is clicked.
		 * 
		 * @property handleElId
		 * @type String
		 * @private
		...
		/**
		 * An object who's property names identify HTML tags to be considered
		 * invalid as drag handles. A non-null property value identifies the tag
		 * as invalid. Defaults to the following value which prevents drag
		 * operations from being initiated by &lt;a> elements:
		 * 
		 * <pre><code>
		 * {
		 * 	A : &quot;A&quot;
		...
		/**
		 * An object who's property names identify the IDs of elements to be
		 * considered invalid as drag handles. A non-null property value
		 * identifies the ID as invalid. For example, to prevent dragging from
		 * being initiated on element ID "foo", use:
		 * 
		 * <pre><code>
		 * {
		 * 	foo : true
		...
		/**
		 * An Array of CSS class names for elements to be considered in valid as
		 * drag handles.
		 * 
		...
		/**
		 * By default, all insances can be a drop target. This can be disabled
		 * by setting isTarget to false.
		 * 
		 * @property isTarget
		 * @type boolean
		 */
		 ...
		 * 
		 * @property padding
		 * @type int[] An array containing the 4 padding values: [top, right,
		 *       bottom, left]
		 */
		padding : null,
		 ...
		 * primary button click (left button for a right-handed mouse). Set to
		 * true to allow drag and drop to start with any mouse click that is
		 * propogated by the browser
		 * 
		...
		/**
		 * By default, drags can only be initiated if the mousedown occurs in
		 * the region the linked element is. This is done in part to work around
		 * a bug in some browsers that mis-report the mousedown if the previous
		...
		/**
		 * Abstract method called after a drag/drop object is clicked and the
		 * drag or mousedown time thresholds have beeen met.
		 * 
		 * @method startDrag
		 * @param {int}
		 *            X click location
		 ...
		 *            INTERSECT mode, an array of one or more dragdrop items
		 *            being hovered over.
		 */
		 ...
		 *            e the mousemove event
		 * @param {String|DragDrop[]}
		 *            id In POINT mode, the element id this was hovering over.
		 *            In INTERSECT mode, an array of dd items that the mouse is
		 *            no longer over.
		 */
		 ...
		 * initial position was determined.
		 * 
		 * @method onAvailable
		 */
		...
		/**
		 * Initializes the drag drop object's constraints to restrict movement
		 ...
		 * 
		 * @param {Mixed}
		 *            constrainTo The element to constrain to.
		 * @param {Object/Number}
		 *            pad (optional) Pad provides a way to specify "padding" of
		 *            the constraints, and can be either a number for
		 *            symmetrical padding (4 would be equal to {left:4, right:4,
		 *            top:4, bottom:4}) or an object containing the sides to
		 *            pad. For example: {right:10, bottom:10}
		 * @param {Boolean}
		 *            inContent (optional) Constrain the draggable in the
		 *            content box of the element (inside padding and borders)
		 */
		 ...
		 * inheritance chain. So a DDProxy implentation will execute apply
		 * config on DDProxy, DD, and DragDrop in order to get all of the
		 * parameters that are available in each object.
		 * 
		 * @method applyConfig
		 */
		 ...
		 * @param id
		 *            the id of the element that will be used to initiate the
		 *            drag
		 */
		 ...
		 * locked (meaning that all drag/drop is disabled on the page.)
		 * 
		 * @method isLocked
		 ...
		 * operation when clicked. This is designed to facilitate embedding
		 * links within a drag handle that do something other than start the
		 * drag.
		 * 
		...
		/**
		 * Create the array of vertical tick marks if an interval was specified
		 * in setYConstraint().
		 * 
		 * @method setYTicks
		 * @private
		 ...
		 * @param {int}
		 *            iLeft the number of pixels the element can move to the
		 *            left
		 * @param {int}
		 *            iRight the number of pixels the element can move to the
		 *            right
		 * @param {int}
		 *            iTickSize optional parameter for specifying that the
...
if (!Ext.dd.DragDropMgr) {
	...
	/**
	 * @class Ext.dd.DragDropMgr DragDropMgr is a singleton that tracks the
	 *        element interaction for all DragDrop items in the window.
	 *        Generally, you will not call this class directly, but it does have
	 *        helper methods that could be useful in your DragDrop
	 *        implementations.
	 * @singleton
	 */
	Ext.dd.DragDropMgr = function() {
		...
		return {
			...
			/**
			 * Flag to determine if we should prevent the default behavior of
			 * the events we define. By default this is true, but this can be
			 * set to false if you need the default behavior (not recommended)
			 * 
			 ...
			 * we generate. This is true by default but you may want to set it
			 * to false if the html element contains other features that require
			 * the mouse click.
			 * 
			...
			/**
			 * Location cache that is set for all drag drop objects when a drag
			 * is initiated, cleared when the drag is finished.
			 * 
			...
			/**
			 * Set useCache to false if you want to force object the lookup of
			 * each drag and drop linked element constantly during a drag.
			 * 
			 * @property useCache
			 * @type boolean
			 * @static
			 ...
			 * the drag if we don't get a mouseup event. Default=1000
			 * 
			 ...
			 * Executed by DragDrop.removeFromGroup, so don't call this function
			 * directly.
			 * 
			 ...
			 * @param {String}
			 *            sHandleId the id of the element that is the drag
			 *            handle
			 * @static
			 */
			 ...
			 * @param {DragDrop}
			 *            the target
			 * @return {boolean} true if the target is a legal target for the dd
			 *         obj
			 * @static
			 */
			 ...
			 * event. Sets up the events required to track the object being
			 * dragged
			 * 
			 * @method handleMouseDown
			 * @param {Event}
			 ...
			 *       can detect this in internet explorer by verifying that the
			 *       mouse is down during the mousemove event. Firefox doesn't
			 *       give us the button state on the mousemove event.
			...
			/**
			 * Refreshes the cache of the top-left and bottom-right points of
			 * the drag and drop objects in the specified group(s). This is in
			 * the format that is stored in the drag and drop instance, so
			 * typical usage is: <code>
			 ...
			 * @return {Ext.lib.Region} a Region object representing the total
			 *         area the element occupies, including any padding the
...
/**
 * @class Ext.dd.DD A DragDrop implementation where the linked element follows
 *        the mouse cursor during a drag.
...
Ext.extend(Ext.dd.DD, Ext.dd.DragDrop, {
	...
	/**
	 * Sets up config options specific to this class. Overrides Ext.dd.DragDrop,
	 * but all versions of this method through the inheritance chain are called
	 */
	applyConfig : function() {
...
/**
 * @class Ext.dd.DDProxy A DragDrop implementation that inserts an empty,
 *        bordered div into the document that follows the cursor during drag
 *        operations. At the time of the click, the frame div is resized to the
 *        dimensions of the linked html element, and moved to the exact location
...
Ext.extend(Ext.dd.DDProxy, Ext.dd.DD, {
			 ...
			 * element we want to drag (this is to get the frame effect). We can
			 * turn it off if we want a different behavior.
			 * 
			...
			/**
			 * By default the frame is positioned exactly where the drag element
			 * is, so we use the cursor offset provided by Ext.dd.DD. Another
			 * option that works only if you do not have constraints on the obj
			 * is to have the drag frame centered around the cursor. Set
			 * centerFrame to true for this effect.
			 * 
			 ...
			 * positions it over the object, and finally displays it
			 * 
			 * @method showFrame
			 * @param {int}
			...
			/**
			 * The proxy is automatically resized to the dimensions of the
			 * linked element when a drag is initiated, unless resizeFrame is
			 * set to false
			 * 
			 * @method _resizeProxy
			 * @private
...
/**
 * @class Ext.dd.DDTarget A DragDrop implementation that does not move, but can
 *        be a drop target. You would get the same result by simply omitting
 *        implementation for the event callbacks, but this way we reduce the
 *        processing cost of the event listener and the callbacks.
 * @extends Ext.dd.DragDrop
 * @constructor
 * @param {String}
