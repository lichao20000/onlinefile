 ...
 * @extends Ext.Toolbar A specialized toolbar that is bound to a
 *          {@link Ext.data.Store} and provides automatic paging controls.
...
Ext.PagingToolbar = Ext.extend(Ext.Toolbar, {
	/**
	 * @cfg {Ext.data.Store} store The {@link Ext.data.Store} the paging toolbar
	 ...
	 */
	/**
	 * @cfg {Number} pageSize The number of records to display per page
	 *      (defaults to 20)
	 */
	pageSize : 20,
	/**
	 * @cfg {String} displayMsg The paging status message to display (defaults
	 *      to "Displaying {0} - {1} of {2}"). Note that this string is
	 *      formatted using the braced numbers 0-2 as tokens that are replaced
	 *      by the values for start, end and total respectively. These tokens
	 *      should be preserved when overriding this string if showing those
	 *      values is desired.
	 */
	...
	/**
	 * Object mapping of parameter names for load calls (defaults to {start:
	 * 'start', limit: 'limit'})
	 */
	paramNames : {
		start : 'start',
