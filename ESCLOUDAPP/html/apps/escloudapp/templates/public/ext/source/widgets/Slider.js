...
/**
 * @class Ext.Slider
 * @extends Ext.BoxComponent Slider which supports vertical or horizontal
 *          orientation, keyboard adjustments, configurable snapping, axis
...
Ext.Slider = Ext.extend(Ext.BoxComponent, {
	/**
	 * @cfg {Number} value The value to initialize the slider with. Defaults to
	 *      minValue.
	 */
	/**
	 * @cfg {Boolean} vertical Orient the Slider vertically rather than
	...
	maxValue : 100,
	/**
	 * @cfg {Number} keyIncrement How many units to change the Slider when
	 *      adjusting with keyboard navigation. Defaults to 1. If the increment
	 *      config is larger, it will be used instead.
	 */
	keyIncrement : 1,
	/**
	 * @cfg {Number} increment How many units to change the slider when
	 *      adjusting by drag and drop. Use this option to enable 'snapping'.
	 */
	increment : 0,
	// private
	clickRange : [5, 15],
	/**
	 * @cfg {Boolean} clickToChange Determines whether or not clicking on the
	 *      Slider axis will change the slider. Defaults to true
	 */
	clickToChange : true,
	/**
	...
	initComponent : function() {
		...
		this.addEvents(
				/**
				 * @event beforechange Fires before the slider value is changed.
				 *        By returning false from an event handler, you can
				 *        cancel the event and prevent the slider from changing.
				 * @param {Ext.Slider}
				 *            slider The slider
				 * @param {Number}
				 *            newValue The new value which the slider is being
				 *            changed to.
				 * @param {Number}
				 *            oldValue The old value which the slider was
				 *            previously.
				 */
				'beforechange',
				/**
				 ...
				 *            slider The slider
				 * @param {Number}
				 *            newValue The new value which the slider has been
				 *            changed to.
				 */
				'change',
				/**
				 * @event changecomplete Fires when the slider value is changed
				 *        by the user and any drag operations have completed.
				 * @param {Ext.Slider}
				 *            slider The slider
				 * @param {Number}
				 *            newValue The new value which the slider has been
				...
				'dragstart',
				/**
				 * @event drag Fires continuously during the drag operation
	 ...
	 * 
	 * @param {Number}
	 *            value The value to set the slider to. (This will be
	 *            constrained within minValue and maxValue)
	 * @param {Boolean}
	...
	/**
	 * Synchronizes the thumb position to the proper proportion of the total
	 * component width based on the current slider {@link #value}. This will be
	 * called automatically when the Slider is resized by a layout, but if it is
	 * rendered auto width, this method can be called from another resize
	 * handler to sync the Slider if necessary.
	 */
	syncThumb : function() {
		if (this.rendered) {
