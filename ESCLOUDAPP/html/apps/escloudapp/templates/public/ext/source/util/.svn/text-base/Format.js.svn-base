...
Ext.util.Format = function() {
	...
	return {
		 ...
		 * @return {Mixed} Empty string if converted, otherwise the original
		 *         value
		 */
		 ...
		 *            value Reference to check
		 * @param {String}
		 *            defaultValue The value to insert of it's undefined
		 *            (defaults to "")
		 * @return {String}
		 */
		 ...
		 *            format expected by the javascript Date object's <a
		 *            href="http://www.w3schools.com/jsref/jsref_parse.asp">parse()</a>
		 *            method)
		 * @param {String}
		 *            format (optional) Any valid date format string (defaults
		 *            to 'm/d/Y')
