 ...
 * @extends Ext.Component Simple color palette class for choosing colors. The
 *          palette can be rendered to any container.<br />
 ...
 * </code></pre>
 * 
 * @constructor Create a new ColorPalette
 * @param {Object}
...
Ext.ColorPalette = function(config) {
	...
	this.addEvents(
			/**
			 * @event select Fires when a color is selected
			 * @param {ColorPalette}
			 *            this
			 * @param {String}
			 *            color The 6-digit color hex code (without the #
			 *            symbol)
			 */
			'select');
...
Ext.extend(Ext.ColorPalette, Ext.Component, {
	/**
	 * @cfg {String} tpl An existing XTemplate instance to be used in place of
	 ...
	 *      6-digit color hex code without the # symbol). Note that the hex
	...
	/**
	 * @cfg {Boolean} allowReselect If set to true then reselecting a color that
	 *      is already selected fires the {@link #select} event
	 */
	allowReselect : false,
	 ...
	 * unique. The width of the palette is controlled via CSS by adjusting the
	 * width property of the 'x-color-palette' class (or assigning a custom
	...
	/**
	 * Selects the specified color in the palette (fires the {@link #select}
	 * event)
	 * 
	 * @param {String}
	 *            color A valid 6-digit color hex code (# will be stripped if
	 *            included)
	 */
