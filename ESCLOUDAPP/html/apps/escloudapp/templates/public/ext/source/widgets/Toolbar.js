...
(function() {
	...
	Ext.extend(T, Ext.BoxComponent, {
		...
		/**
		 * Adds element(s) to the toolbar -- this function takes a variable
		 * number of arguments of mixed type and adds them to the toolbar.
		 * 
		 * @param {Mixed}
		 *            arg1 The following types of arguments are all valid:<br />
		 *            <ul>
		 *            <li>{@link Ext.Toolbar.Button} config: A valid button
		 *            config object (equivalent to {@link #addButton})</li>
		 *            <li>HtmlElement: Any standard HTML element (equivalent to
		 *            {@link #addElement})</li>
		 *            <li>Field: Any form field (equivalent to
		 *            {@link #addField})</li>
		 *            <li>Item: Any subclass of {@link Ext.Toolbar.Item}
		 *            (equivalent to {@link #addItem})</li>
		 *            <li>String: Any generic string (gets wrapped in a
		 *            {@link Ext.Toolbar.TextItem}, equivalent to
		 *            {@link #addText}). Note that there are a few special
		 *            strings that are treated differently as explained next.</li>
		 *            <li>'separator' or '-': Creates a separator element
		 *            (equivalent to {@link #addSeparator})</li>
		...
		/**
		 * Adds a button (or buttons). See {@link Ext.Toolbar.Button} for more
		 * info on the config.
		 * 
		 * @param {Object/Array}
		...
		/**
		 * Inserts any {@link Ext.Toolbar.Item}/{@link Ext.Toolbar.Button} at
		 * the specified index.
		 * 
		 * @param {Number}
		 *            index The index where the item is to be inserted
		 * @param {Object/Ext.Toolbar.Item/Ext.Toolbar.Button/Array}
		 *            item The button, or button config object to be inserted,
		 *            or an array of buttons/configs.
		...
		/**
		 * Adds a dynamically rendered Ext.form field (TextField, ComboBox,
		 * etc). Note: the field should not have been rendered yet. For a field
		 * that has already been rendered, use {@link #addElement}.
		 * 
	 ...
	 * @extends Ext.Toolbar.Item A simple class that adds a vertical separator
	 *          bar between toolbar items. Example usage:
	 * 
	 * <pre><code>
	 * new Ext.Panel({
	 * 			tbar : ['Item 1', {
	 ...
	 *          space between items in a toolbar.
	 * 
	 * <pre><code>
	 * new Ext.Panel({
	 * 			tbar : ['Item 1', {
	 ...
	 *          width) horizontal space between items in a toolbar.
	 * 
	 * <pre><code>
	 * new Ext.Panel({
	 * 			tbar : ['Item 1', {
	 ...
	 * @param {String/Object}
	 *            text A text string, or a config object containing a
	 *            <tt>text</tt> property
	 */
	 ...
	 * @extends Ext.Button A button that renders into a toolbar. Use the
	 *          <tt>handler</tt> config to specify a callback function to
	 ...
	 * @extends Ext.SplitButton A split button that renders into a toolbar.
	 * 
	 * <pre><code>
	 * new Ext.Panel({
	 * 			tbar : [{
	 ...
	 * 											{
	 * 										text : 'Item 1',
	 * 										handler : item1Handler
	 * 									}, {
	 * 										text : 'Item 2',
	 * 										handler : item2Handler
	 * 									}]
	 * 								})
	 * 					}]
	 * 		});
