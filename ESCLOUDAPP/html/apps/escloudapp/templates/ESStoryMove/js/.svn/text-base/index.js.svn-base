...
var _opens = {
	list: function (){
		window.open($.appClient.generateUrl({ESStoryMove:'index',page: _global.page, limit: _global.limit, boardId: _global.boardId}),"_parent")
	},
	lists: function (){ // 仅是列表HTML数据不包含渲染页面
			$.post(
				$.appClient.generateUrl({ESStoryMove:'GetPublishTopicList'},'x'),
	...
	detail: function (info){ // 打开详细页面
		...
		window.open($.appClient.generateUrl({ESStoryMove:'detail_paper',boardId: p[0], topicId: p[1]}),"_parent");	
	}
}
