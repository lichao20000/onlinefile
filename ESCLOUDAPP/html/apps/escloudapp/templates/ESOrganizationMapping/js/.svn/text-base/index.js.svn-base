...
$(function(){
	...
	var setting = {
				...
				async:{
					enable:true,
					url:$.appClient.generateUrl({ESOrganizationMapping:'getnode'},'x'),
					autoParam:["id"]
				},
				callback: {
					onClick: clickNode
	...
	var classic = {
				...
				async:{
					enable:true,
					url:$.appClient.generateUrl({ESOrganizationMapping:'getclassicnode'},'x'),
					autoParam:["id"]
				},
				callback: {
					onClick: clickNode
...
$.getJSON($.appClient.generateUrl({ESOrganizationMapping : "classictree"}, 'x'), function(zNodes) {
...
$.getJSON($.appClient.generateUrl({ESOrganizationMapping : "tree"}, 'x'), function(zNodes) {
	...
	function clickNode(event,treeId,treeNode){
		if($(event.target).closest("div").attr("id")== "catagory"){
			...
			var tableurl=$.appClient.generateUrl({ESOrganizationMapping:'organ_json',trid:treeid},'x');
...
function addorgan()
{
	...
	var url = $.appClient.generateUrl({ESOrganizationMapping:'organadd'},'x');
	$.ajax({
	    url:url,
	    success:function(data){
	    	$.dialog({
			    ...
			    ok:function(){
			    	...
			    	if(classic==""){
			    		$.dialog.notice({icon : 'warning',content : '请选择分类树',title : '3秒后自动关闭',time : 3});
			    		return false;
			    	}
			    	var data = $("#addorgan").serialize();
			    	var url = $.appClient.generateUrl({ESOrganizationMapping : 'addval'}, 'x');
...
function delorgan(){
	...
	$.dialog({
			...
			cancelVal:'取消',
			ok:function(){
				var url=$.appClient.generateUrl({ESOrganizationMapping:'delval'},'x');
...
function save(){
	...
	var url=$.appClient.generateUrl({ESOrganizationMapping:'saveval'},'x');
...
function filter(){
	...
	$.ajax({
	    url:$.appClient.generateUrl({ESOrganizationMapping:'filter'},'x'),
	    success:function(data){
	    	$.dialog({
		    	title:'筛选面板',
		    	lock:true,
			    ...
			    ok:function(){
			    	...
			    	var url = $.appClient.generateUrl({ESOrganizationMapping:'filter_json',trid:treeid},'x');
