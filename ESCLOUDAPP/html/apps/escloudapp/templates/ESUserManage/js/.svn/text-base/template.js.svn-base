	...
	display: function (){
		...
		if(user){
			...
			var checkeds = $('#u_list input[name="checks"]:checked'),cl = checkeds.length;
				if(!cl){
					$.dialog.notice({content: '请选择授权用户', icon: 'warning', time: 2, lock: false});
					return;
				}
				...
				var data = {accountIds: accountIds, roleIds: roleIds};
					...
					$.post(
						...
						function (isok){
							...
							if(isok){
								$.dialog.notice({content: '授权成功', icon: 'succeed', time: 2, lock: false});
							}else{
								$.dialog.notice({content: '授权失败', icon: 'error', time: 2, lock: false});
							}
						}
					);
...
window.onload = function (){
	$("#estabs").esTabs("open", {title:"用户管理", content:"#ESSystemIndex"});
	$("#estabs").esTabs("select", "用户管理");
