...
function  add_fileEquipment(){
	var appId = 0;
	var treeObj = $.fn.zTree.getZTreeObj("fileEquipAppTreeDemo");
	if(treeObj!=null){
	...
	$.ajax({
			...
			ESFileEquipment : 'add_fileEquipment',appId:appId},'x'),
...
function edit_fileEquipment(tr){
	var columns = ['appId','id','ip','port','rootDir','isEnabled','priority','description'];
	var colValues = $("#fileEquipGrid").flexGetColumnValue(tr,columns);
		$.ajax({
		    url : $.appClient.generateUrl({ESFileEquipment : 'edit_fileEquipment',data:colValues},'x'),
		    success:function(data){
		    editdia=$.dialog({
			    	title:'编辑文件存储设备',
		    	   	fixed:false,
...
function keyWordQuery(){
	...
	$("#fileEquipGrid").flexOptions({url:url}).flexReload();
	return false;
}
