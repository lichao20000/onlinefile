...
////	var tplPath =  "/apps/escloudapp/templates";
//	mxClient.link('stylesheet', tplPath+'/ESWorkflow/css/workflow/explorer.css');
}
...
var mxClipboard = {
	...
	cut : function(graph, cells) {
			...
			//return cells;
			//xiaoxiong 20120910 添加一个ajax请求 调用verificationIsHasNotDealWf方法获取所要剪切组建是否存在正在活动的流程 如果存在 则给出提示 不允许剪切
			...
			if(modelID == ''){
				...
				$.post( $.appClient.generateUrl({ESWorkflow : 'verificationIsHasNotDealWf'}, 'x')
...

{
	...
	mxCellEditor.prototype.startEditing = function(cell, trigger) {
		...
		if (state != null) {
			...
			this.textarea.onkeyup =function() {
				...
				} else if(getRealLength(this.value) > 64){
					this.style.border = '1px solid red';
					this.title = '该输入项的最大长度为64' ;
	...
	mxCellEditor.prototype.stopEditing = function(cancel) {
		...
		//if(this.textarea.value.trim().length==0 || this.textarea.value.length>64 || this.textarea.value.indexOf("'")>-1 || this.textarea.value.indexOf("\"")>-1 || this.textarea.value.indexOf("\\")>-1){
		...
		if (state.style[mxConstants.STYLE_FONTSIZE] > 0
			...
			state.text = new mxText(value, new mxRectangle(),
					...
					isEdge || isForceHtml, graph.isWrapping(state.cell), graph
							.isLabelClipped(state.cell),state.cell.getId());//////jiang 20100203
	...
	mxGraphHandler.prototype.mouseMove = function(sender, me) {
		if (!me.isConsumed() && this.cell != null && this.shape != null
			...
			if (this.shape.node.style.visibility == 'visible'
				...
				/*** xiaoxiong 20120509 添加鼠标是否已经移动到编辑窗体的最左侧或者最上侧 如果是的话 将其组建移动到相应的最边上 不要移出编辑窗体之外 start ***/
				...
				/*** xiaoxiong 20120509 添加鼠标是否已经移动到编辑窗体的最左侧或者最上侧 如果是的话 将其组建移动到相应的最边上 不要移出编辑窗体之外 end ***/
...

{
	...
	mxVertexHandler.prototype.union = function(bounds, dx, dy, index) {
		if (this.singleSizer) {
			...
			/*** xiaoxiong 20120608 添加鼠标是否已经移动到编辑窗体的最左侧或者最上侧 如果是的话 将其组建移动到相应的最边上 不要移出编辑窗体之外 start ***/
			if(left<0){
				if(top<0){
			...
			/*** xiaoxiong 20120608 添加鼠标是否已经移动到编辑窗体的最左侧或者最上侧 如果是的话 将其组建移动到相应的最边上 不要移出编辑窗体之外 end ***/
		}
	};
	...
	mxEdgeHandler.prototype.mouseMove = function(sender, me) {
		if (this.index != null && this.marker != null) {
			...
			if (this.isLabel) {
				...
				//xiaoxiong 20120510 分支不能直接连接到结束或开始节点 (ellipse - > 开始 结束 ；rhombus - > 分支)
...
////////////////////********************************************************//
////////////////////********************************************************//
// 选择关联业务结构 jiang 20091020
	function selectStructure(formId,textFieldId){
	    new Ext.Window({ 
	     id:'seletePKG_window', 
	     title :'选择要导入的结构', 
	     modal  : true, 
	     width : 850, 
	     height : 550, 
                  autoLoad:{url:'forwardUtil.html?content.method=selectPkgPath',params:{roleId:1,formId:formId ,textFieldId:textFieldId},nocache:true,scripts:true}, 
                  autoScroll:true
		 }) 
         Ext.getCmp('seletePKG_window').show();
	}
	// role grid click
	function rolegridClickEvent(selectionModel, rowIndex, selectedRecord){
		var select = Ext.getCmp('osModel_step_Role_select').getSelectionModel().getSelections();
//		if(select[0]==null){roleId = '';return false;}
		//chenjian 20121024 当没选择节点的时候，将搜索框的值也清空，保持原状
		if(select[0]==null){Ext.getCmp('osModel_step_user_select').getTopToolbar().items.get(1).setValue('');}
		roleId = '';
		for(i = 0; i < select.length; i++){
			roleId += select[i].data.id + ',';
		}
		wfUserRoleListSearchKeyWord("", roleId);
	}
//	选择角色到右边去 //20100422 jiang
	function addRolesToSelect(selectionModel, rowIndex, selectedRecord){
		var select = Ext.getCmp('osModel_step_Role_select').getSelectionModel().getSelections();
	    var smodel = Ext.getCmp('osModel_step_user_selected').getSelectionModel();
	    var store = Ext.getCmp('osModel_step_user_selected').getStore();
	    if(!select[0]) {showMsg('请先勾选你要选择的数据!','3');return false;}
		if(select[0].data==null){return false;}
		var hasedUserNames = "" ;//xiaoxiong 20120326 添加 用于记载那些已经被选择
		for(var i=0;i<select.length;i++){
			var bool = true;
			var r=new Ext.data.Record(select[i].data);
			for(var j=0;j<store.getCount();j++){
				var cur=store.getAt(j);
				if(r.get('roleId')==cur.get('roleId')){
					bool = false;
				}
			}
			if(bool){
			 	var selectStore = Ext.getCmp('osModel_step_user_selected').getStore();
			 	selectStore.insert(store.getCount(),r);
			} else {
				hasedUserNames = hasedUserNames + r.get("name") + "、" ;//xiaoxiong 20120326 添加 用于记载那些已经被选择
			}
		}
		//xiaoxiong 20120326 当含有已经被选择的角色时 给出提示
		if(hasedUserNames != ''){
			showMsg(hasedUserNames.substring(0,hasedUserNames.length-1)+'已选择！','3');
		}
		smodel.selectAll();
	}
//	选择用户到右边去
	function addUsersToSelect(selectionModel, rowIndex, selectedRecord){
		var select = Ext.getCmp('osModel_step_user_select').getSelectionModel().getSelections();
		if(select[0]==null){return false;}
		var store = Ext.getCmp('osModel_step_user_selected').getStore();
		for(var i=0;i<select.length;i++){
			var r=new Ext.data.Record(select[i].data);
			var check=0;
			for(var j=0;j<store.getCount();j++){
				var cur=store.getAt(j);
				//jiangyuntao 20130707 edit 修复bug6680，区分开用户和角色
				if(!cur.get('roleId') && r.get('id')==cur.get('id')){
					check=1;
				}
			}
							     //alert(r.get('name'));
...
//	删除已经选择的用户和角色
	function deleteSelectUsersOrRoles(selectionModel, rowIndex, selectedRecord){
		var select = Ext.getCmp('osModel_step_user_selected').getSelectionModel().getSelections();
		if(select[0]==null){return false;}
		var store = Ext.getCmp('osModel_step_user_selected').getStore();
		for(var i=0;i<select.length;i++){
			 store.remove(select[i]);
//			 store.reload();
		}
		//xiaoxiong 20110919 将剩余的自动选中
		if(store.getTotalCount() > 0){
			Ext.getCmp('osModel_step_user_selected').getSelectionModel().selectAll();
		}
		//end
	}
	function stepRoleSelect(radio,checked){
		if(checked){
			roleId = '' ;
			var ISCOUNTERSIGN=false;
			if(Ext.getCmp('osModel_step_user_select_main_panel_organ')){
				// jiang add 20100625
				ISCOUNTERSIGN = Ext.getCmp('ES_STEP_ISCOUNTERSIGN').getValue();
				Ext.getCmp('ES_MODEL_STEP_SELECT_NEXT_OWER').remove(Ext.getCmp('osModel_step_user_select_main_panel_organ'));
			}
			if(!Ext.getCmp('osModel_step_Role_select')){
				// 角色
				var record = [{name:'id'},{name:'name'},{name:'roleId'}];
				//var column = [{header:'id',dataIndex:'id',hidden: 'true'},{header:'角色名称',dataIndex:'name'}];
				//var column = new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel(),{header:'id',dataIndex:'id',hidden: 'true'},{header:'角色',dataIndex:'name',width:230},{header:'roleId',dataIndex:'roleId',hidden: 'true'}]);
				//xiaoxiong 20110917 将表格的列模型设置为可以换行显示 添加 renderer , renderer : function(value, meta, record) {meta.attr = 'style="white-space:normal;"'; return value;}
				var column = new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel(),{header:'id',dataIndex:'id',hidden: 'true'},{header:'角色',dataIndex:'name',width:480 , renderer : function(value, meta, record) {meta.attr = 'style="white-space:normal;"'; return value;} },{header:'roleId',dataIndex:'roleId',hidden: 'true'}]);
				/**将store移出来，方便分页调用*/
				var storeRole =new Ext.data.Store({
				   		 proxy:new Ext.data.HttpProxy({url:$.appClient.generateUrl({ESWorkflow : 'getAllRoles'}, 'x')}),
				   		 baseParams : { searchKeyword:"",start:0,limit:10 },
				   		 reader:new Ext.data.JsonReader(
				   		 	{totalProperty:'size',root:'list'},record
				   		 )
				   	});
				var rolegrid = new Ext.grid.GridPanel({
					id:'osModel_step_Role_select',
				   	store:storeRole,
				    //cm:new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel(),column]),
				   	cm:column,
				    sm: new Ext.grid.CheckboxSelectionModel(),
				    width:480,height:170,
				    autoScroll:true,region:'north',/**zhengfang 20130609 去掉frame:true,使页面布局更简洁，组件四周没有条条框框,使转发页面更美观**/
				    tbar:new Ext.Toolbar({items:['&nbsp;&nbsp;搜索词:',createSearchRoleFieldFun('osModel_step_Role_select'),'->',{text:'选择',iconCls:'arrow_right',handler:addRolesToSelect}]}),
				    listeners : {click:rolegridClickEvent},
			     	/**chenjian 20130320 增加分页功能，防止用户量过多而引发异常*/
			     	bbar:new Ext.PagingToolbar({
			    		pageSize:10,
			    		id:'showStepRoleGridpageBar',
			    		store:storeRole,
			    		displayInfo:true
			   	 	})
				});
				rolegrid.getStore().load({params : {start : 0,limit : 10}});
				// 用户
				var usercolumn = new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel(),{header:'id',dataIndex:'id',hidden: 'true'},{header:'用户名称',dataIndex:'name',width:80},{header:'机构',dataIndex:'organ',width:230}]);
				var usergrid = new Ext.grid.GridPanel({
					id:'osModel_step_user_select',
				   	store:new Ext.data.Store(),
				    cm:usercolumn,
				    sm: new Ext.grid.CheckboxSelectionModel(),
				     enableDragDrop : true, stripeRows  : true,
				    width:480, height:170,
				    autoScroll:true,/**zhengfang 20130609 去掉frame:true,使页面布局更简洁，组件四周没有条条框框,使转发页面更美观**/
				    region: 'center',
				    tbar:new Ext.Toolbar({items:[
				    //fuhongyi 20100928 按角色
				    '&nbsp;&nbsp;搜索词:',new Ext.app.SearchField({id:'inputField'+i+'_serarchKeyword',name:'userSearchKeyword', width:130,   
		                                                       onTrigger2Click:function(){
		                                                      	 		userList_serarchKeyword=Ext.getCmp(this.id).getValue();
		  															  //if (null == userList_serarchKeyword || userList_serarchKeyword.trim() == ''){showMsg('"+PkgTagUtils.message(pageContext, "OsPkgGrid.prompt.title")+"', '"+PkgTagUtils.message(pageContext, "Search.inputTitle")+"');return ;}
		                                                       			if (null == userList_serarchKeyword || userList_serarchKeyword.trim() == ''){showMsg('请输入搜索词','3');return ;}//yuzhijia 20101102 edit 上边那种方式在js中不管用
		                                                       			this.triggers[0].show();this.hasSearch = false; 
				                                                       	wfUserRoleListSearchKeyWord(userList_serarchKeyword, roleId);
																},
															   	onTrigger1Click:function(){
															   				userList_serarchKeyword ='';
															   				wfUserRoleListSearchKeyWord(userList_serarchKeyword, roleId);
															   			this.triggers[0].hide();this.el.dom.value = '';this.hasSearch = false; 
												                      
												                      }
		                                                 }) ,
		               //fuhongyi 20100928 关键词检索 end
				    '->',{text:'选择',iconCls:'arrow_right',handler:addUsersToSelect}]}),
				    /**chenjian 20121108 增加分页功能，防止用户量过多而引发异常*/
				    bbar:new Ext.PagingToolbar({
				    	pageSize:20,
				    	id:'showUserGridpageBar',
				    	store:new Ext.data.Store(),
				    	displayInfo:true
				    })
				});
				usergrid.on('celldblclick',addUsersToSelect);
				// 已经选择的用户或角色
				//var userSelectedColumn =  new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel(),//jiang add 20100422
				//	{header:'id',dataIndex:'id',hidden: 'true'},{header:'名称',dataIndex:'name',width:230},{header:'roleId',dataIndex:'roleId',hidden: 'true'}]);
				//xiaoxiong 20110917 将表格的列模型设置为可以换行显示 添加 renderer
				var userSelectedColumn =  new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel(),
					{header:'id',dataIndex:'id',hidden: 'true'},{header:'名称',dataIndex:'name',width:230,renderer:function(value, meta, record) {meta.attr = 'style="white-space:normal;"'; return value;}},{header:'roleId',dataIndex:'roleId',hidden: 'true'}]);
				var store=new Ext.data.Store({
					proxy:new Ext.data.HttpProxy({url:$.appClient.generateUrl({ESWorkflow : 'showCurrStepUsers'}, 'x')}),
					reader:new Ext.data.JsonReader({
					totalProperty:'size',root:'list'},
					[{name:'id'},{name:'name'},{name:'roleId'}]//jiang add 20100422
					)
				});
				store.on('load',function(){Ext.getCmp('osModel_step_user_selected').getSelectionModel().selectAll();});
				store.load({ 
			  		params : { 
			   		start : 0, 
			        limit : 10 ,
			        wfmodelID:radio.wfmodelid,
			        wfstepID:radio.stepid
			   }
			 }); 
			 	
				var userSelectedgrid = new Ext.grid.GridPanel({
					id:'osModel_step_user_selected',
				   	//store:new Ext.data.SimpleStore({fields:['id','name'],data:[] }),
					store:store,
				    cm:userSelectedColumn,
				    sm: new Ext.grid.CheckboxSelectionModel(),
				    enableDragDrop : true, stripeRows : true,
				    width:250,height:340,deferRowRender:false,
				    autoScroll:true,/**zhengfang 20130609 去掉frame:true,使页面布局更简洁，组件四周没有条条框框,使转发页面更美观**/
				    region:'center',
				    tbar:new Ext.Toolbar({items:[{text:'去掉',iconCls:'arrow_left',handler:deleteSelectUsersOrRoles}
				    ,'->'
					,{
						id:'ES_STEP_ISCOUNTERSIGN',xtype : 'checkbox', boxLabel:'是否会签',hideLabel :true ,name:'ES_STEP_ISCOUNTERSIGN'
					}
				    ]})
				});
				userSelectedgrid.on('celldblclick',deleteSelectUsersOrRoles);
				//Ext.getCmp('osModel_step_user_selected').store.on('load',function(s,r,o){this.getSelectionModel().selectAll();});
				var panel = new Ext.Panel({
					id:'osModel_step_user_select_panel',
				    width:480, height:340,
				    layout : 'border',
				    region: 'west',
				    items:[rolegrid,usergrid]
				});
				var mainpanel = new Ext.Panel({
					id:'osModel_step_user_select_main_panel',
				    width:800,height:350,
				    layout : 'border',
				    items:[panel,userSelectedgrid]
				});
				Ext.getCmp('ES_MODEL_STEP_SELECT_NEXT_OWER').add(mainpanel);
				Ext.getCmp('ES_MODEL_STEP_SELECT_NEXT_OWER').doLayout();
				Ext.getCmp('ES_STEP_ISCOUNTERSIGN').setValue(ISCOUNTERSIGN);
				wfUserRoleListSearchKeyWord(userList_serarchKeyword, roleId);
			}
		}
	}
	////按机构选择用户
	function stepOrganSelect(radio,checked){
		if(checked){
		/**liuhuifang 20130312 添加机构默认刷全部数据*/
		/**chenjian 20130419 modify 参数值修改为一个固定值,如果为变量的话，当用户第一次进来时点击了机构树，当第二次进入界面时,数据刷新错误*/
			var ISCOUNTERSIGN=false;
			if(Ext.getCmp('osModel_step_user_select_main_panel')){
				// jiang add 20100625
				if(Ext.getCmp('ES_STEP_ISCOUNTERSIGN'))ISCOUNTERSIGN = Ext.getCmp('ES_STEP_ISCOUNTERSIGN').getValue();
				Ext.getCmp('ES_MODEL_STEP_SELECT_NEXT_OWER').remove(Ext.getCmp('osModel_step_user_select_main_panel'));
			}
				//Ext.getCmp('ES_MODEL_STEP_SELECT_NEXT_OWER').remove(Ext.getCmp('osModel_step_user_select_main_panel'));
				var wfModelStep_Select_Organ_Tree = new Ext.tree.TreePanel({   
		 	       id:'wfModelStep_Select_Organ_Tree',       
		 	       autoScroll:true, animate:false, enableDD:false,     
		 	       containerScroll: true, height:200,region:'center',
		 	       tbar:new Ext.Toolbar({items:['->']}),
		 	       border:false,/**zhengfang 20130609 增加border:false,使页面布局更简洁，组件四周没有条条框框,使转发页面更美观**/
		 	       loader: new Ext.tree.TreeLoader({ dataUrl:'' })     
		 	   });  
		 	   var wfModelStep_Select_Organ_root = new Ext.tree.AsyncTreeNode({     
		 	       text: '系统机构', draggable:false, id:'-1'     
		 	   });     
		 	   wfModelStep_Select_Organ_Tree.setRootNode(wfModelStep_Select_Organ_root);  
		 	   //xiaoxiong 20100629 edit
		 	   //wfModelStep_Select_Organ_Tree.on('beforeload', function(node){wfModelStep_Select_Organ_Tree.loader.dataUrl = 'treeUtil.html?content.method=getOrganTreeNodes'; });  
		 	   /** jiangyuntao 20130712 edit 修改为不走分级过滤 **/
//		 	   wfModelStep_Select_Organ_Tree.on('beforeload', function(node){wfModelStep_Select_Organ_Tree.loader.dataUrl = 'treeUtil.html?content.method=getOrganTreeNodes4SetWf&wfModel=wfModel'; });      
		 	   wfModelStep_Select_Organ_Tree.on('beforeload', function(node){wfModelStep_Select_Organ_Tree.loader.dataUrl = $.appClient.generateUrl({ESWorkflow : 'getOrganTreeNodes4SetWf',wfModel:'wfModel',node:node.id},'x'); });      
		 	   wfModelStep_Select_Organ_Tree.on('click', function(node, e){ 
		 	   	/**chenjian 20130411 BUGID:7180 将下面一行代码注释掉*/
		 	   	 	/**由于存在BUG：7246将其重新放开*/
		 	   		nodeChecked = node.id;
					wfUserOrganListSearchKeyWord(nodeChecked, "");
		 	   });     
		 	   wfModelStep_Select_Organ_root.expand();   
		 	   	// 用户
//jiangyuntao 20110826 edit 增加显示机构
				var usercolumn = new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel(),{header:'id',dataIndex:'id',hidden: 'true'},{header:'用户名称',dataIndex:'name',width:80},{header:'机构',dataIndex:'organ',width:230}]);
				var usergrid = new Ext.grid.GridPanel({
					id:'osModel_step_user_select_organ',
				   	store:new Ext.data.Store({
				   		 id:'osModel_step_user_select_organ_store',
				   		 proxy:new Ext.data.HttpProxy({url:$.appClient.generateUrl({ESWorkflow : 'getUserFromOrgan'}, 'x')}),
				   		 baseParams: { searchKeyword : '',id:'', start:0,limit:10 },//送参数，关键字
				   		 reader:new Ext.data.JsonReader(
				   		 	{totalProperty:'size',root:'list'},[{'name':'id'},{'name':'name'},{'name':'organ'}]
				   		 )
				   	}),
				    cm:usercolumn,
				    sm: new Ext.grid.CheckboxSelectionModel(),
				    ...
				    '&nbsp;&nbsp;搜索词:',new Ext.app.SearchField({id:'inputField'+i+'_serarchKeyword',name:'userSearchKeyword', width:130,   
		                                                       onTrigger2Click:function(){
		  															    ...
		  															    //if (null == userList_serarchKeyword || userList_serarchKeyword.trim() == ''){showMsg('"+PkgTagUtils.message(pageContext, "OsPkgGrid.prompt.title")+"', '"+PkgTagUtils.message(pageContext, "Search.inputTitle")+"');return ;}
		                                                       			if (null == userList_serarchKeyword || userList_serarchKeyword.trim() == ''){showMsg('请输入搜索词','3');return ;}//yuzhijia 20101102 edit 上边那种方式在js中不管用
		                                                       			this.triggers[0].show();this.hasSearch = false; 
		                                                       			/**liuhuifang 20130311 修改根据机构选择用户选择顶层节点检索后取消检索的问题*/
		                                                       			if(nodeChecked == 1 || nodeChecked == 2){
				                                                       		wfUserOrganListSearchKeyWord('', userList_serarchKeyword);
		                                                       			}else{
				                                                       		wfUserOrganListSearchKeyWord(nodeChecked, userList_serarchKeyword);
		                                                       			}
																},
															   	onTrigger1Click:function(){
															   				userList_serarchKeyword ='';
		               ...
		               //fuhongyi 20100928 关键词检索 end
				    '->',{text:'选择',iconCls:'toRight',handler:
						function (selectionModel, rowIndex, selectedRecord){
							var select = Ext.getCmp('osModel_step_user_select_organ').getSelectionModel().getSelections();
							if(select[0]==null){return false;}
							var store = Ext.getCmp('osModel_step_user_selected_organ').getStore();
							//for(var i=0;i<select.length;i++){
							//	 var r=new Ext.data.Record(select[i].data);
							//	 store.insert(store.getCount(),r);
							//}
							...
							for(var i=0;i<select.length;i++){
								 ...
								 if(check!=1){
									 ...
									 if( '' == msg){
									 	msg = select[i].data.name ;
									 } else {
									 	msg = msg + ';' + select[i].data.name ;
									 }
								}
							...
							var smodel=Ext.getCmp('osModel_step_user_selected_organ').getSelectionModel();
							smodel.selectAll();
						}
					}]}),/**chenjian 20121108 增加分页功能，防止用户量过多而引发异常*/
					 bbar:new Ext.PagingToolbar({
				    	pageSize:10,
				    	id:'showOrganGridpageBar',
				    	store:Ext.StoreMgr.get('osModel_step_user_select_organ_store'),
				    	displayInfo:true
				    })
				});
				usergrid.on('celldblclick',function(selectionModel, rowIndex, selectedRecord){
					var select = Ext.getCmp('osModel_step_user_select_organ').getSelectionModel().getSelections();
							if(select[0]==null){return false;}
							var store = Ext.getCmp('osModel_step_user_selected_organ').getStore();
							for(var i=0;i<select.length;i++){
				...
				// 已经选择的用户或角色
				//var userSelectedColumn = [{header:'id',dataIndex:'id',hidden: 'true'},{header:'',dataIndex:'name',width:230}];
				// jiang add 20100422
				//var userSelectedColumn = new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel(),{header:'id',dataIndex:'id',hidden: 'true'},{header:'名称',dataIndex:'name',width:230},{header:'roleId',dataIndex:'roleId',hidden: 'true'}]);
				//xiaoxiong 20110917 将表格的列模型设置为可以换行显示 添加 renderer
				var userSelectedColumn = new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel(),{header:'id',dataIndex:'id',hidden: 'true'},{header:'名称',dataIndex:'name',width:230,renderer:function(value, meta, record) {meta.attr = 'style="white-space:normal;"'; return value;}},{header:'roleId',dataIndex:'roleId',hidden: 'true'}]);
				var store=new Ext.data.Store({
					proxy:new Ext.data.HttpProxy({url:$.appClient.generateUrl({ESWorkflow : 'showCurrStepUsers'}, 'x')}),
					baseParams:{
					   		start : 0, 
				...
				store.on('load',function(){Ext.getCmp('osModel_step_user_selected_organ').getSelectionModel().selectAll();});
				store.load(); 
				var userSelectedgrid = new Ext.grid.GridPanel({
					id:'osModel_step_user_selected_organ',
			//	store:new Ext.data.SimpleStore({fields:['id','name'],data:[]
//				   				,listeners:{load :function(s,r,o){   
//									    var selectionModel = Ext.getCmp('osModel_step_user_selected_organ').getSelectionModel();
...
//								}} 
				 //  	}),
				    ...
				    cm:userSelectedColumn,
				    sm: new Ext.grid.CheckboxSelectionModel(),
				    width:250,height:400,deferRowRender:false,
				    autoScroll:true,/**zhengfang 20130609 去掉frame:true,使页面布局更简洁，组件四周没有条条框框,使转发页面更美观**/
				    region:'center',
				    tbar:new Ext.Toolbar({items:[{text:'去掉',iconCls:'toLeft',handler:
						function (selectionModel, rowIndex, selectedRecord){
							var select = Ext.getCmp('osModel_step_user_selected_organ').getSelectionModel().getSelections();
							if(select[0]==null){return false;}
							var store = Ext.getCmp('osModel_step_user_selected_organ').getStore();
							for(var i=0;i<select.length;i++){
								 store.remove(select[i]);
							...
							//xiaoxiong 20110919 将剩余的自动选中
							if(store.getTotalCount() > 0){
								Ext.getCmp('osModel_step_user_selected_organ').getSelectionModel().selectAll();
							}
							//end
						}
					},'->'
				...
				userSelectedgrid.on('celldblclick',function(selectionModel, rowIndex, selectedRecord){
					var select = Ext.getCmp('osModel_step_user_selected_organ').getSelectionModel().getSelections();
							if(select[0]==null){return false;}
							var store = Ext.getCmp('osModel_step_user_selected_organ').getStore();
							for(var i=0;i<select.length;i++){
				...
				var treePanel = new Ext.Panel({
					id:'osModel_step_user_select_treePanel_organ',
				    width:250, height:200,
				    layout : 'border',
				    region:'north',
				    items:[wfModelStep_Select_Organ_Tree]
				});
				var panel = new Ext.Panel({
					id:'osModel_step_user_select_panel_organ',
				...
				var mainpanel = new Ext.Panel({
					id:'osModel_step_user_select_main_panel_organ',
				...
				Ext.getCmp('ES_MODEL_STEP_SELECT_NEXT_OWER').doLayout();
		 	   	// jiang add 20100625
				if(Ext.getCmp('ES_STEP_ISCOUNTERSIGN'))Ext.getCmp('ES_STEP_ISCOUNTERSIGN').setValue(ISCOUNTERSIGN);
				wfUserOrganListSearchKeyWord(1, "");
			}
//			if(Ext.getCmp('osModel_step_user_select')){
//				Ext.getCmp('ES_MODEL_STEP_SELECT_NEXT_OWER').remove(Ext.getCmp('osModel_step_user_select'));
//				Ext.getCmp('ES_MODEL_STEP_SELECT_NEXT_OWER').doLayout();
//			}
//		}
	}
	
	//jiangyuntao 20100129  选择函数双向列表右侧单击事件处理方法
	...
	$.post( $.appClient.generateUrl({ESWorkflow : 'formCheckMethod'}, 'x')
				...
				if(functionid == '1'){
					...
					//	handler : function(){
							...
							//var thisbs=Ext.getCmp('updateStateFunction').getValue();
		...
		function deleteCellfromDB(cellId,graph){
			...
			//end
			//xiaoxiong 20120516 添加判断 当时删除步骤时 添加验证要删除步骤在系统中是否存在流转的数据 如果存在不允许删除 否则可以
				if(others == ''){
					var postData = "edges="+edges+"&others="+others+"&modelID="+modelID ;
					$.post( $.appClient.generateUrl({ESWorkflow : 'deleteCellfromDB'}, 'x')
							,{data:postData}, function(res){
								var json = eval('(' + res + ')');
								...
								if(json.msgType == "1"){
									$.dialog.notice({icon : 'succeed',content : json.msg,title : '3秒后自动关闭',time : 3});
								} else {
		...
		function deleteWorkflowByCloseWindow(){
			$.post( $.appClient.generateUrl({ESWorkflow : 'dropWfModel'}, 'x')
				,{modelId:Ext.getCmp('osWfModel_custom_form_init_hidden_Model_ID').getValue()}, function(res){
					var json = eval('(' + res + ')');
					...
					if("1" == json.msgType){
						$.dialog.notice({icon : 'succeed',content : json.message,title : '3秒后自动关闭',time : 3});
					} else {
		...
		function saveWorkflowGraphXml(graph){
				...
				for(var i=0;i<cells.length;i++){
					...
					if(style!=null&&style.length>=7&&style.substring(0,7)=='ellipse'&&isEdge==false&&cellValue=='开始'){///  开始节点（椭圆形）
						beginCount++;
						for(var j=0;j<edges.length;j++){
							var edgecell=edges[j];
							var sourceCell = edgecell.getTerminal(true);
							var targetCell = edgecell.getTerminal(false);
							//alert('cellID:'cell.getId()+",   sourceID:"+sourceCell.getId()+",    targetCell:"+targetCell.getId());
...
//		    	postData += "&username="+$("#createWorkflowDiv").attr("userName") ;
        		$.post($.appClient.generateUrl({ESWorkflow : 'saveWfModel'}, 'x')
		    			,{data:postData,
        					graphXml:graphXml,
        					modelId:modelId ,
	        				...
	        				if (json.success == 'true') {
				        		...
				        		if(returnMsg){
				        			$.dialog.notice({icon : 'succeed',content : '保存成功！<br>'+returnMsg,title : '3秒后自动关闭',time : 3});
				        		}else{
...
//jiang add private ext 20091019
//Ext.OaisViewport = Ext.extend(Ext.Container, {
...
//    }
//});
//Ext.reg('OaisViewport', Ext.OaisViewport);

/****lihui 20130409 协同管理-新建事项定制流程选择角色界面中增加检索功能***/
var setworkfolowModelserachRolekeyword = '';
function createSearchRoleFieldFun(gridID){          
return new Ext.app.SearchField({id:'setworkfolowModel_search_role_button', width:130,disabled :false,
	onTrigger2Click:function(){
		if(!(this.el.dom.value&&this.el.dom.value.trim()!='')){ 
			showMsg('请先输入关键词且不能全为空格，再进行此操作！','3'); 
			return; 
		}
		setworkfolowModelserachRolekeyword=this.el.dom.value;
		querySerachRoleFieldData(gridID);
		this.triggers[0].show();this.hasSearch = false; 
	} 
	,onTrigger1Click:function(){ 
		setworkfolowModelserachRolekeyword='';
		querySerachRoleFieldData(gridID);
		this.triggers[0].hide();this.el.dom.value = '';this.hasSearch = false;
	}
}); 
}
function querySerachRoleFieldData(gridID){
	var record = [{name:'id'},{name:'name'},{name:'roleId'}];
	var column = new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel('osModel_step_Role_select'),{header:'id',dataIndex:'id',hidden: 'true'},{header:'角色',dataIndex:'name',width:230, renderer : function(value, meta, record) {meta.attr = 'style="white-space:normal;"'; return value;}},{header:'roleId',dataIndex:'roleId',hidden: 'true'}]);//xiaoxiong 20111010 添加换行显示样式	
	var curr_dataStore= new Ext.data.Store({      
		proxy:new Ext.data.HttpProxy({url:$.appClient.generateUrl({ESWorkflow : 'getAllRoles'}, 'x')}),
		baseParams:{searchKeyword:setworkfolowModelserachRolekeyword, start:0,limit:10 },//增加分页参数
		reader:new Ext.data.JsonReader({
			totalProperty:'size',root:'list'},record)
	});
	var curr_GridPanel = Ext.getCmp(gridID);
	curr_GridPanel.reconfigure(curr_dataStore, column);
	Ext.getCmp('showStepRoleGridpageBar').bind(curr_dataStore);
	curr_dataStore.load();
}
/***lihui end***/
//xiaoxiong 20100630 edit
var roleId = '';
function AssignedWorkflowStepRoleSelect(radio,checked){
		if(checked){
			roleId = '' ;
			var ISCOUNTERSIGN=false;
			if(Ext.getCmp('osModel_step_user_select_main_panel_organ')){
			...
			if(!Ext.getCmp('osModel_step_Role_select')){
				// 角色
				var record = [{name:'id'},{name:'name'},{name:'roleId'}];
				var column = new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel(),{header:'id',dataIndex:'id',hidden: 'true'},{header:'角色',dataIndex:'name',width:480},{header:'roleId',dataIndex:'roleId',hidden: 'true'}]);
				/**chenjian 20130320 将store移出来，方便分页调用*/
				var storeRole =new Ext.data.Store({
				   		 proxy:new Ext.data.HttpProxy({url:$.appClient.generateUrl({ESWorkflow : 'getAllRoles'}, 'x')}),
				   		 params : { searchKeyword:"",start:0,limit:10},/**chenjian 20130320 增加分页参数*/
				   		 reader:new Ext.data.JsonReader(
				   		 	{totalProperty:'size',root:'list'},record
				   		 )
				});
				var rolegrid = new Ext.grid.GridPanel({
				    ...
				    width:480,height:170,
				    autoScroll:true,region:'north',/**zhengfang 20130609 去掉frame:true,使页面布局更简洁，组件四周没有条条框框,使转发页面更美观**/
				   // tbar:new Ext.Toolbar({items:['->',{text:'选择',iconCls:'arrow_right',handler:addRolesToSelect}]}),
				     /***lihui 20130409 协同管理-新建事项定制流程按角色选择界面中 增加搜索功能***/    
				    tbar:new Ext.Toolbar({items:['&nbsp;&nbsp;搜索词:',createSearchRoleFieldFun('osModel_step_Role_select'),'->',{text:'选择',iconCls:'arrow_right',handler:addRolesToSelect}]}),
				    listeners : {click:rolegridClickEvent},
				     /**chenjian 20130320 增加分页功能，防止用户量过多而引发异常*/
				    bbar:new Ext.PagingToolbar({
...
//jiangyuntao 20110826 edit 增加显示机构
				...
				var userSelectedgrid = new Ext.grid.GridPanel({
				    ...
				    sm: new Ext.grid.CheckboxSelectionModel(),
				    enableDragDrop : true, stripeRows : true,
				    width:250,height:338,deferRowRender:false,
				    autoScroll:true,/**zhengfang 20130609 去掉frame:true,使页面布局更简洁，组件四周没有条条框框,使转发页面更美观**/
				    region:'center',
				    tbar:new Ext.Toolbar({items:[{text:'去掉',iconCls:'arrow_left',handler:deleteSelectUsersOrRoles}
				    ]})
				...
				var panel = new Ext.Panel({
				    ...
				    layout : 'border',
				    region: 'west',
				    border:false,/**zhengfang 20130626 修改定制流程页面布局和样式**/
				    items:[rolegrid,usergrid]
				});
				var mainpanel = new Ext.Panel({
					id:'osModel_step_user_select_main_panel',
	...
	////按机构选择用户
	//fuhongyi 20100819 add 添加userSelected参数,用户自动选中
//	function AssignedWorkflowStepOrganSelect(radio,checked){
	var nodeChecked = '1';
	var userList_serarchKeyword ='';
	function AssignedWorkflowStepOrganSelect(radio,checked,userSelected){
		if(checked){
		/**liuhuifang 20130312 添加机构默认刷全部数据*/
		/**chenjian 20130419 modify 参数值修改为一个固定值,如果为变量的话，当用户第一次进来时点击了机构树，当第二次进入界面时,数据刷新错误*/
			wfUserOrganListSearchKeyWord(1, "");
			...
			if(Ext.getCmp('osModel_step_user_select_main_panel')){
				...
				var wfModelStep_Select_Organ_Tree = new Ext.tree.TreePanel({   
		 	       id:'wfModelStep_Select_Organ_Tree',       
		 	       autoScroll:true, animate:false, enableDD:false,     
		 	       containerScroll: true, height:142,region:'center',
		 	       loader: new Ext.tree.TreeLoader({ dataUrl:'' })     
		 	   });  
		 	   var wfModelStep_Select_Organ_root = new Ext.tree.AsyncTreeNode({     
		 	       text: '系统机构', draggable:false, id:'-1'     
		 	   ...
		 	   wfModelStep_Select_Organ_Tree.setRootNode(wfModelStep_Select_Organ_root);  
		 	   /** jiangyuntao 20130712 edit 修改为不走分级过滤 **/
//		 	   wfModelStep_Select_Organ_Tree.on('beforeload', function(node){wfModelStep_Select_Organ_Tree.loader.dataUrl = 'treeUtil.html?content.method=getOrganTreeNodes4SetWf&wfModel=wfModel&isNewEvent:true'; });      
		 	   wfModelStep_Select_Organ_Tree.on('beforeload', function(node){wfModelStep_Select_Organ_Tree.loader.dataUrl = $.appClient.generateUrl({ESWorkflow : 'getOrganTreeNodes4SetWf',wfModel:'wfModel',node:node.id},'x'); });      
		 	   wfModelStep_Select_Organ_Tree.on('click', function(node, e){ 
		 	   	/**chenjian 20130411 BUGID:7180 将下面一行代码注释掉*/
		 	   	 	/**由于存在BUG：7246将其重新放开*/
		 	   		nodeChecked = node.id; // fuhongyi 20100928
					wfUserOrganListSearchKeyWord(nodeChecked, "");
		 	   });      
		 	   wfModelStep_Select_Organ_root.expand();   
		 	   	// 用户
//jiangyuntao 20110826 edit 增加显示机构
				...
				var usergrid = new Ext.grid.GridPanel({
				    ...
				    '&nbsp;&nbsp;搜索词:',new Ext.app.SearchField({id:'inputField'+i+'_serarchKeyword',name:'userSearchKeyword', width:130,   
		               ...
		               //fuhongyi 20100928 关键词检索 end
				    '->',{text:'选择',iconCls:'arrow_right',handler:
						function (selectionModel, rowIndex, selectedRecord){
							var select = Ext.getCmp('osModel_step_user_select_organ').getSelectionModel().getSelections();
							if(select[0]==null){return false;}
							var store = Ext.getCmp('osModel_step_user_selected_organ').getStore();
					...
					bbar:new Ext.PagingToolbar({//chenjian 20121023 增加分页
				    	pageSize:10,
				    	id:'showOrganGridpageBar',
				    	store:Ext.StoreMgr.get('osModel_step_user_select_organ_store'),
				    	displayInfo:true
				...
				var store=new Ext.data.Store({
					proxy:new Ext.data.HttpProxy({url:$.appClient.generateUrl({ESWorkflow : 'showCurrStepUsers'}, 'x')}),
					baseParams:{userSelected:userSelected},
					reader:new Ext.data.JsonReader({
					totalProperty:'size',root:'list'},
					[{name:'id'},{name:'name'},{name:'roleId'}])
				});
				...
				var userSelectedgrid = new Ext.grid.GridPanel({
				    ...
				    cm:userSelectedColumn,
				    sm: new Ext.grid.CheckboxSelectionModel(),
				    width:250,height:380,deferRowRender:false,
				    autoScroll:true,/**zhengfang 20130609 去掉frame:true,使页面布局更简洁，组件四周没有条条框框,使转发页面更美观**/
				    region:'center',
				    tbar:new Ext.Toolbar({items:[{text:'去掉',iconCls:'arrow_left',handler:
						function (selectionModel, rowIndex, selectedRecord){
							var select = Ext.getCmp('osModel_step_user_selected_organ').getSelectionModel().getSelections();
							if(select[0]==null){return false;}
							var store = Ext.getCmp('osModel_step_user_selected_organ').getStore();
...
//end	
	//------------------------------------------
   	...
   	//jiangjien20100629
   	//------------------------------------------
	function rolegridForActionClickEvent(selectionModel, rowIndex, selectedRecord){
		var select = Ext.getCmp('osModel_step_Role_select').getSelectionModel().getSelections();
		if(select[0]==null){//xiaoxiong 20120709 取消选择时 将下面显示的用户信息清空
			var userGrid = Ext.getCmp('osModel_step_user_select');
			userGrid.reconfigure(new Ext.data.Store({}), userGrid.getColumnModel());
	...
	function addRolesToSelectForActionEvent(selectionModel, rowIndex, selectedRecord){
		var select = Ext.getCmp('osModel_step_Role_select').getSelectionModel().getSelections();
	    var smodel = Ext.getCmp('osModel_step_user_selected').getSelectionModel();
	    var store = Ext.getCmp('osModel_step_user_selected').getStore();
		if(select[0].data==null){return false;}
		var oldHasSelectedRoles = '' ;
		for(var i=0;i<select.length;i++){
			var bool = true;
			var r=new Ext.data.Record(select[i].data);
			for(var j=0;j<store.getCount();j++){
			...
			if(bool){
			 ...
			 selectStore.insert(store.getCount(),r);
				...
				oldHasSelectedRoles = oldHasSelectedRoles +　r.get('name') + '、'　;
				//oldHasSelectedRoles = oldHasSelectedRoles + '【'+oldHasSelectedUsers.substring(0,oldHasSelectedUsers.length-1)+'】已经被选择，不能重复选择！' ;
			}
		}
		if(oldHasSelectedRoles.length>0){
			showMsg('【'+oldHasSelectedRoles.substring(0,oldHasSelectedRoles.length-1)+'】已经被选择，不能重复选择！','3');
		}
		smodel.selectAll();
	}
//	选择用户到右边去
	function addUsersToSelectForActionEvent(selectionModel, rowIndex, selectedRecord){
		var select = Ext.getCmp('osModel_step_user_select').getSelectionModel().getSelections();
		if(select[0]==null){return false;}
		var store = Ext.getCmp('osModel_step_user_selected').getStore();
		
		
		var oldHasSelectedUsers = '' ;
		for(var i=0;i<select.length;i++){
			var r=new Ext.data.Record(select[i].data);
			var check=0;
			for(var j=0;j<store.getCount();j++){
				var cur=store.getAt(j);
				//if(r.get('id')==cur.get('id')){
				var roleId = cur.get('roleId') ;
				if((roleId=='' || typeof(roleId) == 'undefined') && r.get('id')==cur.get('id')){
					check=1;
					break;
				}
			}
			...
			if(check!=1){
				...
				oldHasSelectedUsers = oldHasSelectedUsers +　r.get('name') + '、'　;
			}
			var smodel=Ext.getCmp('osModel_step_user_selected').getSelectionModel();
			smodel.selectAll();
		}
		if(oldHasSelectedUsers.length>0){
			showMsg((oldHasSelectedUsers.length>0?('【'+oldHasSelectedUsers.substring(0,oldHasSelectedUsers.length-1)+'】已经被选择，不能重复选择！'):'','3'));
		}
	}
//	删除已经选择的用户和角色
	function deleteSelectUsersOrRolesForActionEvent(selectionModel, rowIndex, selectedRecord){
		var select = Ext.getCmp('osModel_step_user_selected').getSelectionModel().getSelections();
		if(select[0]==null){return false;}
		var store = Ext.getCmp('osModel_step_user_selected').getStore();
		for(var i=0;i<select.length;i++){
...
//			 store.reload();
		...
		//xiaoxiong 201200710 取消后将剩余的全部选中
		var smodel=Ext.getCmp('osModel_step_user_selected').getSelectionModel();
		smodel.selectAll();
	}
	// role
	function stepRoleSelectForActionEvent(radio,checked){
		if(checked){
			roleId = '' ;
			if(Ext.getCmp('osModel_step_user_select_main_panel')){
				Ext.getCmp('ES_MODEL_STEP_SELECT_NEXT_OWER').remove(Ext.getCmp('osModel_step_user_select_main_panel'));
			...
			if(!Ext.getCmp('osModel_step_Role_select')){
				...
				var record = [{name:'id'},{name:'name'},{name:'roleId'}];
				//var column = [{header:'id',dataIndex:'id',hidden: 'true'},{header:'角色名称',dataIndex:'name'}];
				//var column = new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel(),{header:'id',dataIndex:'id',hidden: 'true'},{header:'角色',dataIndex:'name',width:180},{header:'roleId',dataIndex:'roleId',hidden: 'true'}]);
				//xiaoxiong 20111028 将表格的列模型设置为可以换行显示 添加 renderer 
				var column = new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel(),{header:'id',dataIndex:'id',hidden: 'true'},{header:'角色',dataIndex:'name',width:480, renderer : function(value, meta, record) {meta.attr = 'style="white-space:normal;"'; return value;}},{header:'roleId',dataIndex:'roleId',hidden: 'true'}]);
				/**chenjian 20130320 将store移出来，方便分页调用*/
				var storeRole =new Ext.data.Store({
				   		 proxy:new Ext.data.HttpProxy({url:$.appClient.generateUrl({ESWorkflow : 'getAllRoles'}, 'x')}),
				   		 baseParams : { searchKeyword:"",start:0,limit:10 },/**chenjian 20130320 增加分页参数*/
				   		 reader:new Ext.data.JsonReader(
				   		 	{totalProperty:'size',root:'list'},record
				   		 )
				   	});
				var rolegrid = new Ext.grid.GridPanel({
				    ...
				    width:480,height:170,
				    autoScroll:true,region:'north',/**zhengfang 20130609 去掉frame:true,使页面布局更简洁，组件四周没有条条框框,使转发页面更美观**/
				    tbar:new Ext.Toolbar({items:['&nbsp;&nbsp;搜索词:',createSearchRoleFieldFun('osModel_step_Role_select'),'->',{text:'选择',iconCls:'arrow_right',handler:addRolesToSelectForActionEvent}]}),
				    listeners : {click:rolegridForActionClickEvent},
				     /**chenjian 20130320 增加分页功能，防止用户量过多而引发异常*/
				     bbar:new Ext.PagingToolbar({
				    	pageSize:10,
				    	id:'showStepRoleGridpageBar',
...
//jiangyuntao 20110826 edit 增加显示机构
				...
				var usergrid = new Ext.grid.GridPanel({
				    ...
				    region: 'center',
				    tbar:new Ext.Toolbar({items:[
				    //jiangyuntao 20110817 edit 设置知会人中增加搜索用户
				    '&nbsp;&nbsp;搜索词:',new Ext.app.SearchField({id:'inputField'+i+'_serarchKeyword',name:'userSearchKeyword', width:130,   
		                                                       onTrigger2Click:function(){
		                                                      	 		userList_serarchKeyword=Ext.getCmp(this.id).getValue();
		  															  //if (null == userList_serarchKeyword || userList_serarchKeyword.trim() == ''){showMsg('"+PkgTagUtils.message(pageContext, "OsPkgGrid.prompt.title")+"', '"+PkgTagUtils.message(pageContext, "Search.inputTitle")+"');return ;}
				    ...
				    '->',{text:'选择',iconCls:'arrow_right',handler:addUsersToSelectForActionEvent}]}),
				    /**chenjian 20121108 增加分页功能，防止用户量过多而引发异常*/
				     bbar:new Ext.PagingToolbar({
				    	pageSize:10,
				    	id:'showUserGridpageBar',
				...
				usergrid.on('celldblclick',addUsersToSelectForActionEvent);
				// 已经选择的用户或角色
				//var userSelectedColumn =  new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel(),//jiang add 20100422
				//	{header:'id',dataIndex:'id',hidden: 'true'},{header:'名称',dataIndex:'name',width:180},{header:'roleId',dataIndex:'roleId',hidden: 'true'}]);
				//xiaoxiong 20111028 将表格的列模型设置为可以换行显示 添加 renderer 
				var userSelectedColumn =  new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel(),//jiang add 20100422
					{header:'id',dataIndex:'id',hidden: 'true'},{header:'名称',dataIndex:'name',width:180, renderer : function(value, meta, record) {meta.attr = 'style="white-space:normal;"'; return value;}},{header:'roleId',dataIndex:'roleId',hidden: 'true'}]);
				var store=new Ext.data.Store({
					//listeners:{load:function(a,b,c){Ext.getCmp('osModel_step_user_selected').getSelectionModel().selectAll();}},
					proxy:new Ext.data.HttpProxy({url:$.appClient.generateUrl({ESWorkflow : 'getNoticeUsers'}, 'x')}),
					baseParams:{start : 0, 
				        limit : 10 ,
				        wfmodelID:radio.wfmodelid,
				        actionId:radio.actionId},
					reader:new Ext.data.JsonReader({
					totalProperty:'size',root:'list'},
					[{name:'id'},{name:'name'},{name:'roleId'}]//jiang add 20100422
					)
			...
			var userSelectedgrid = new Ext.grid.GridPanel({
			    ...
			    sm: new Ext.grid.CheckboxSelectionModel(),
			    enableDragDrop : true, stripeRows : true,
			    width:Ext.getCmp('ES_MODEL_STEP_SELECT_NEXT_OWER').getInnerWidth()-250,height:Ext.getCmp('ES_MODEL_STEP_SELECT_NEXT_OWER').getInnerHeight(),deferRowRender:false,
			    autoScroll:true,border : false,
			    region:'center',
			    tbar:new Ext.Toolbar({items:[{text:'去掉',iconCls:'arrow_left',handler:deleteSelectUsersOrRolesForActionEvent}
			    ]})
			});
			userSelectedgrid.on('celldblclick',deleteSelectUsersOrRolesForActionEvent);
			var panel = new Ext.Panel({
				id:'osModel_step_user_select_panel',
			    width:480, height:Ext.getCmp('ES_MODEL_STEP_SELECT_NEXT_OWER').getInnerHeight(),
			    layout : 'border',
			...
			var mainpanel = new Ext.Panel({
				id:'osModel_step_user_select_main_panel',
			    width:Ext.getCmp('ES_MODEL_STEP_SELECT_NEXT_OWER').getInnerWidth(),height:Ext.getCmp('ES_MODEL_STEP_SELECT_NEXT_OWER').getInnerHeight(),
	...
	///organ 设置动作属性中的设置知会人
	function stepOrganSelectForActionEvent(radio,checked){
		if(checked){
		/**liuhuifang 20130312 添加机构默认刷全部数据*/
		/**chenjian 20130419 modify 参数值修改为一个固定值,如果为变量的话，当用户第一次进来时点击了机构树，当第二次进入界面时,数据刷新错误*/
//			wfUserOrganListSearchKeyWord(1, "");alert(2);
			if(Ext.getCmp('osModel_step_user_select_main_panel')){
				Ext.getCmp('ES_MODEL_STEP_SELECT_NEXT_OWER').remove(Ext.getCmp('osModel_step_user_select_main_panel'));
			}
			if(Ext.getCmp('osModel_step_user_select_main_panel_organ')){
				...
				var wfModelStep_Select_Organ_Tree = new Ext.tree.TreePanel({   
		 	       id:'wfModelStep_Select_Organ_Tree',       
		 	       autoScroll:true, animate:false, enableDD:false,     
		 	       containerScroll: true, height:140,region:'center',
		 	       tbar:new Ext.Toolbar({items:['->']}),
		 	       loader: new Ext.tree.TreeLoader({ dataUrl:'' })     
		 	   });  
		 	   var wfModelStep_Select_Organ_root = new Ext.tree.AsyncTreeNode({     
		 	   ...
		 	   wfModelStep_Select_Organ_Tree.setRootNode(wfModelStep_Select_Organ_root);  
		 	   /** jiangyuntao 20130712 edit 修改为不走分级过滤 **/
//		 	   wfModelStep_Select_Organ_Tree.on('beforeload', function(node){wfModelStep_Select_Organ_Tree.loader.dataUrl = 'treeUtil.html?content.method=getOrganTreeNodes4SetWf'; });      
		 	   wfModelStep_Select_Organ_Tree.on('beforeload', function(node){wfModelStep_Select_Organ_Tree.loader.dataUrl = $.appClient.generateUrl({ESWorkflow : 'getOrganTreeNodes4SetWf',wfModel:'wfModel',node:node.id},'x'); });      
		 	   wfModelStep_Select_Organ_Tree.on('click', function(node, e){
		 	   	/**chenjian 20130411 BUGID:7180 将下面一行代码注释掉*/
		 	   	 	/**由于存在BUG：7246将其重新放开*/
...
//jiangyuntao 20110826 edit 增加显示机构
				...
				var usergrid = new Ext.grid.GridPanel({
				    ...
				    sm: new Ext.grid.CheckboxSelectionModel(),
				    width:250, height:200,
				    autoScroll:true,border : false,/**zhengfang 20130609 去掉frame:true,使页面布局更简洁，组件四周没有条条框框,使转发页面更美观**/
				    region: 'center',
				    tbar:new Ext.Toolbar({items:[
                    //jiangyuntao 20110817 edit 为设置知会人增加检索
				     '&nbsp;&nbsp;搜索词:',new Ext.app.SearchField({id:'inputField'+i+'_serarchKeyword',name:'userSearchKeyword', width:130,   
		                                                       onTrigger2Click:function(){
		                                                      	 		userList_serarchKeyword=Ext.getCmp(this.id).getValue();
		  															    //if (null == userList_serarchKeyword || userList_serarchKeyword.trim() == ''){showMsg('"+PkgTagUtils.message(pageContext, "OsPkgGrid.prompt.title")+"', '"+PkgTagUtils.message(pageContext, "Search.inputTitle")+"');return ;}
				...
				// 已经选择的用户或角色
				//var userSelectedColumn = new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel(),{header:'id',dataIndex:'id',hidden: 'true'},{header:'名称',dataIndex:'name',width:180},{header:'roleId',dataIndex:'roleId',hidden: 'true'}]);
				//xiaoxiong 20111028 将表格的列模型设置为可以换行显示 添加 renderer 
				var userSelectedColumn = new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel(),{header:'id',dataIndex:'id',hidden: 'true'},{header:'名称',dataIndex:'name',width:180, renderer : function(value, meta, record) {meta.attr = 'style="white-space:normal;"'; return value;}},{header:'roleId',dataIndex:'roleId',hidden: 'true'}]);
				var store=new Ext.data.Store({
					proxy:new Ext.data.HttpProxy({url:$.appClient.generateUrl({ESWorkflow : 'getNoticeUsers'}, 'x')}),
					baseParams:{start : 0, 
				        limit : 10 ,
				...
				var mainpanel = new Ext.Panel({
					id:'osModel_step_user_select_main_panel_organ',
				    width:Ext.getCmp('ES_MODEL_STEP_SELECT_NEXT_OWER').getInnerWidth(),height:Ext.getCmp('ES_MODEL_STEP_SELECT_NEXT_OWER').getInnerHeight(),
				    layout : 'border',border : false,clearCls:'allowFloat',
				    items:[panel,userSelectedgrid]
				});
				Ext.getCmp('ES_MODEL_STEP_SELECT_NEXT_OWER').add(mainpanel);
				Ext.getCmp('ES_MODEL_STEP_SELECT_NEXT_OWER').doLayout();
	...
	//xiaoxiong 20100920 add 
		function ForwardWfStepRoleSelect(radio,checked){
		if(checked){
			roleId = '' ;
			var ISCOUNTERSIGN=false;
			if(Ext.getCmp('osModel_step_user_select_main_panel_organ')){
			...
			if(!Ext.getCmp('osModel_step_Role_select')){
				...
				var rolegrid = new Ext.grid.GridPanel({
					id:'osModel_step_Role_select',
				   	store:storeRole,
				   	cm:column,
				   	/**zhengfang 20130614 列宽强制自适应，使页面更美观***/
				   	viewConfig:{forceFit:true},
				    sm: new Ext.grid.CheckboxSelectionModel(),
				    width:480,height:175,
				    autoScroll:true,region:'north',
				    //tbar:new Ext.Toolbar({items:['->',{text:'选择',iconCls:'arrow_right',handler:addRolesToSelect}]}),
				    /***lihui 20130409 协同管理-新建事项 定制流程 按角色选择界面中 增加搜索功能***/
				    /** jiangyuntao 20130710 edit 去掉选择，转发时不提供按角色转发 **/
				    tbar:new Ext.Toolbar({items:['&nbsp&nbsp搜索词:',createSearchRoleFieldFun('osModel_step_Role_select')]}),
				    listeners : {click:rolegridClickEvent},
			     	/**chenjian 20130320 增加分页功能，防止用户量过多而引发异常*/
			     	bbar:new Ext.PagingToolbar({
			    		pageSize:10,
				...
				// 用户
//jiangyuntao 20110826 edit 增加显示机构
				var usercolumn = new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel(),{header:'id',dataIndex:'id',hidden: 'true'},{header:'用户名称',dataIndex:'name',width:80},{header:'机构',dataIndex:'organ',width:170}]);
				var usergrid = new Ext.grid.GridPanel({
					id:'osModel_step_user_select',
				   	store:new Ext.data.Store(),
				    cm:usercolumn,
	...
	////按机构选择用户
	function ForwardWfStepOrganSelect(radio,checked){
		if(checked){
			var ISCOUNTERSIGN=false;
		/**liuhuifang 20130312 添加机构默认刷全部数据*/
		/**chenjian 20130419 modify 参数值修改为一个固定值,如果为变量的话，当用户第一次进来时点击了机构树，当第二次进入界面时,数据刷新错误*/
			...
			if(Ext.getCmp('osModel_step_user_select_main_panel')){
				...
				var wfModelStep_Select_Organ_Tree = new Ext.tree.TreePanel({   
		 	       id:'wfModelStep_Select_Organ_Tree',       
		 	       autoScroll:true, animate:false, enableDD:false,     
		 	       containerScroll: true, height:156,region:'center',
		 	       /**zhengfang 20130614 tbar上任何按钮都没有，将其注释掉，使布局更好看**/
		 	       /**tbar:new Ext.Toolbar({items:['->']}),**/
		 	       loader: new Ext.tree.TreeLoader({ dataUrl:'' })     
		 	   });  
		 	   var wfModelStep_Select_Organ_root = new Ext.tree.AsyncTreeNode({     
...
//		 	   wfModelStep_Select_Organ_Tree.on('beforeload', function(node){wfModelStep_Select_Organ_Tree.loader.dataUrl = 'treeUtil.html?content.method=getOrganTreeNodes4SetWf&wfModel=wfModel'; });      
		 	   ...
		 	   wfModelStep_Select_Organ_Tree.on('click', function(node, e){ 
		 	   	/**chenjian 20130411 BUGID:7180 将下面一行代码注释掉*/
		 	   	/**由于存在BUG：7246将其重新放开*/
		 	   		nodeChecked = node.id; // fuhongyi 20100928 机构id
					wfUserOrganListSearchKeyWord(nodeChecked, "");
		 	   });      
		 	   wfModelStep_Select_Organ_root.expand();   
		 	   	// 用户
//jiangyuntao 20110826 edit 增加显示机构
				/**zhengfang 20130614 调整ColumnModel中的width，使页面更美观**/
				var usercolumn = new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel(),{header:'id',dataIndex:'id',hidden: 'true'},{header:'用户名称',dataIndex:'name',width:80},{header:'机构',dataIndex:'organ',width:170}]);
				var usergrid = new Ext.grid.GridPanel({
					id:'osModel_step_user_select_organ',
				   	store:new Ext.data.Store({
	...
	//点击机构 检索用户
	function wfUserOrganListSearchKeyWord(id, userList_serarchKeyword){
			Ext.StoreMgr.get('osModel_step_user_select_organ_store').baseParams={searchKeyword:userList_serarchKeyword,id:id, start:0,limit:10};
			Ext.StoreMgr.get('osModel_step_user_select_organ_store').load();
	}
	//点击角色 检索用户
	function wfUserRoleListSearchKeyWord(userList_serarchKeyword, roleId){
			var userGrid = Ext.getCmp('osModel_step_user_select');
			var userrecord = [{'name':'id'},{'name':'name'},{'name':'organ'}];
			var store = new Ext.data.Store({
			   		 proxy:new Ext.data.HttpProxy({url:$.appClient.generateUrl({ESWorkflow : 'getUserFromRole'}, 'x')}),
			   		 baseParams: { searchKeyword : userList_serarchKeyword,roleId:roleId, start:0,limit:10 },//送参数，关键字
			   		 reader:new Ext.data.JsonReader(
			   		 	{totalProperty:'size',root:'list'},userrecord
			   		 )
			});
			userGrid.reconfigure(store, userGrid.getColumnModel());
			Ext.getCmp('showUserGridpageBar').bind(store);
			userGrid.getStore().load();
    }
//fuhongyi 20100928 end 