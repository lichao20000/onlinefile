...
(function($){
	function validateOnChange(e)
	{
		...
		var freg=new RegExp("^\d+\.\d{"+dolength+"}$");
		...
		switch(reg)
		{
			...
			case 'float': var fregNew=new RegExp("^\d+(\.\d{1,"+dolength+"})?$"); msg='格式为浮点类型且最多有'+dolength+'位小数';reg =fregNew;break;
			case 'phone':msg = '手机号码格式不正确！请重新输入！';reg = /^(?:13\d|15\d)\d{5}(\d{3}|\*{3})$/;break;
		}
        ...
        if(value!=''){
        	...
        	if(strlength > len ){
        		if(null != isPhone && isPhone == "phone"){
        			$e.attr('title','手机号长度最大为'+len+'个数字');
        			$e.addClass("invalid-text");
        			return false;
        		}
        		$e.addClass("invalid-text");
