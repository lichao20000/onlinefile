(function($, window, document, undefined) {
	...
	var methods = {
		...
		init : function(options) {
			options = $
					.extend(true, {}, $.fn.pagewalkthrough.defaults, options);
			var that = this;
			if (!options.name) {
	...
	function showStep(skipDirection) {
		var options = _activeWalkthrough, step = options.steps[_index], targetElement = options._element
				.find(step.wrapper), scrollTarget = getScrollParent(targetElement), maxScroll, scrollTo;
		...
		scrollTo = step.popup.type === "modal" ? 0 : Math.floor(targetElement
				.offset().top
				- $(window).height() / 2 + scrollTarget.scrollTop());
		if (scrollTarget.scrollTop() !== scrollTo
				&& (scrollTarget.scrollTop() === maxScroll
						&& scrollTo < maxScroll || scrollTo <= 0
						&& scrollTarget.scrollTop() > 0 || scrollTo > 0)) {
	...
	function buildWalkthrough() {
		...
		if (step.popup.type !== "modal" && step.popup.type !== "nohighlight") {
			...
			if (step.wrapper === "" || typeof step.wrapper === "undefined") {
				debug('Your walkthrough position is: "'
						+ step.popup.type
			...
			maxHeight = maxHeight <= 0 ? targetElement.outerHeight()
					: maxHeight;
			$jpwOverlay.appendTo($jpWalkthrough);
			$("<div>").addClass("overlay-hole").height(
					Math.min(maxHeight, targetElement.outerHeight())+15).width(
					targetElement.outerWidth()).css({
				padding : "0px",
				position : "absolute",
				top : targetElement.offset().top -5 ,
	...
	function showModal() {
		...
		var tooltipSlide = $('<div id="tooltipTop">'
				...
				+ "</div>" + '<div id="tooltipInner">' + "</div>"
				+ '<div id="tooltipBottom">' + '<div id="bottomLeft"></div>'
				+ '<div id="bottomRight"></div>' + "</div>");
		$jpWalkthrough.html("");
	...
	function showTooltip() {
		...
		var top, left, arrowTop, arrowLeft, overlayHoleWidth = $(
				"#jpWalkthrough .overlay-hole").outerWidth(), overlayHoleHeight = $(
				"#jpWalkthrough .overlay-hole").outerHeight(), overlayHoleTop = $(
				"#jpWalkthrough .overlay-hole").offset().top, overlayHoleLeft = $(
		...
		$("#tooltipWrapper").css(textRotation);
		$("#tooltipInner").append(getContent(step)).show();
		$jpwTooltip.append('<span class="' + step.popup.position
				+ '">&nbsp;</span>');
		switch (step.popup.position) {
		case "top":
			top = overlayHoleTop - ($jpwTooltip.height() + arrow / 2)
					+ parseInt(step.popup.offsetVertical, 10) - 86;
			left = overlayHoleLeft + overlayHoleWidth / 2 - $jpwTooltip.width()
					/ 2 - 5 + parseInt(step.popup.offsetHorizontal, 10);
			arrowLeft = $jpwTooltip.width() / 2 - arrow
					+ parseInt(step.popup.offsetArrowHorizontal, 10);
			arrowTop = step.popup.offsetArrowVertical ? parseInt(
					step.popup.offsetArrowVertical, 10) : "";
			break;
		case "right":
			...
			arrowLeft = step.popup.offsetArrowHorizontal ? parseInt(
					step.popup.offsetArrowHorizontal, 10) : "";
			break;
		case "bottom":
			top = overlayHoleTop + overlayHoleHeight
					+ parseInt(step.popup.offsetVertical, 10) + 86;
			left = overlayHoleLeft + overlayHoleWidth / 2 - $jpwTooltip.width()
					/ 2 - 5 + parseInt(step.popup.offsetHorizontal, 10)-130;
			arrowLeft = $jpwTooltip.width() / 2 - arrow
					+ parseInt(step.popup.offsetArrowHorizontal, 10);
			arrowTop = step.popup.offsetArrowVertical ? parseInt(
					step.popup.offsetArrowVertical, 10) : "";
		...
		case "left":
			...
			left = overlayHoleLeft - $jpwTooltip.width() - arrow
					+ parseInt(step.popup.offsetHorizontal, 10) - 105;
			arrowTop = arrow + parseInt(step.popup.offsetArrowVertical, 10);
			arrowLeft = step.popup.offsetArrowVertical ? parseInt(
					step.popup.offsetArrowHorizontal, 10) : "";
			break;
		case "bottom-left":
			top = overlayHoleTop + overlayHoleHeight
	...
	function getScrollParent(element) {
		...
		var position = element.css("position"), excludeStaticParent = position === "absolute", scrollParent = element
				.parents().filter(
						function() {
							var parent = $(this);
		...
		return position === "fixed" ? $() : !scrollParent.length ? $("body")
				: scrollParent
	}
	...
	$.pagewalkthrough = $.fn.pagewalkthrough = function(method) {
		if (methods[method]) {
			...
			$.error("Method " + method
					+ " does not exist on jQuery.pagewalkthrough")
		}
	};
