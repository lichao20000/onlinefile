...
ChinaUnicom.page = {
  ...
  tabController: {
    ...
    resize: function() {
      ...
      self.dNavBar.children('div').width(eachWidth).children('a.floatLeft').width(aEachWidth);
    },
...
function search(type){
  ...
  if(type === 'common'){
    ...
    if (a === 'info') {
      ...
      var bundle = $.trim($('#bundle').val());
      if(create_name.length === 0 && bundle.length === 0){
        alert('请输入搜索内容');
        return;
      }
      id = 'searchInfoBox';
...
function alert(content){
  ...
  left = parseInt(left*100);
  //top = parseInt(top*100);
  msg = '<div class="error" style="width:310px;">';
  ...
  msg += '<div class="errorText">'+content+'</div>';
